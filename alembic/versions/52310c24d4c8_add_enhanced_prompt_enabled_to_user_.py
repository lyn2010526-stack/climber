...
def _invalid_autogenerated_upgrade() -> None:
    ...
    sa.Column('is_force_delivery', sa.Boolean(), nullable=False),
    ...
    sa.Column('admin_tags', sa.JSON(), nullable=False),
    sa.PrimaryKeyConstraint('id')
    )
...
def _invalid_autogenerated_downgrade() -> None:
    # ### commands auto generated by Alembic - please adjust! ###
...
def upgrade() -> None:
    """Apply only the schema delta from the initial revision."""
    op.create_table(
        "auto_loop_tasks",
        sa.Column("id", sa.String(length=36), nullable=False),
        sa.Column("objective", sa.Text(), nullable=False),
    ...
    op.create_index("ix_auto_loop_tasks_heartbeat_at", "auto_loop_tasks", ["heartbeat_at"])
    op.create_index("ix_auto_loop_tasks_status", "auto_loop_tasks", ["status"])
    ...
    op.create_index("ix_archival_passages_user_id", "archival_passages", ["user_id"])
    ...
    op.create_index("ix_core_memory_blocks_agent_id", "core_memory_blocks", ["agent_id"])
    op.create_index("ix_core_memory_blocks_user_id", "core_memory_blocks", ["user_id"])
    ...
    with op.batch_alter_table("mcp_servers") as batch_op:
        ...
        batch_op.add_column(sa.Column("tools", sa.JSON(), server_default=sa.text("'[]'"), nullable=False))
    with op.batch_alter_table("skills") as batch_op:
        batch_op.add_column(sa.Column("updated_at", sa.DateTime(), server_default=sa.text("(CURRENT_TIMESTAMP)"), nullable=False))
        batch_op.add_column(sa.Column("scope", sa.String(length=20), server_default="global", nullable=False))
        batch_op.add_column(sa.Column("scope_id", sa.String(length=36), nullable=True))
        batch_op.add_column(sa.Column("is_force_delivery", sa.Boolean(), server_default=sa.false(), nullable=False))
        batch_op.add_column(sa.Column("is_orphan", sa.Boolean(), server_default=sa.false(), nullable=False))
        ...
        batch_op.add_column(sa.Column("admin_tags", sa.JSON(), server_default=sa.text("'[]'"), nullable=False))
        batch_op.create_index("ix_skills_scope_id", ["scope_id"])
    ...
    with op.batch_alter_table("agent_group_tasks") as batch_op:
        batch_op.add_column(sa.Column("dependencies", sa.JSON(), server_default=sa.text("'[]'"), nullable=False))
...
def downgrade() -> None:
    ...
    with op.batch_alter_table("skills") as batch_op:
        batch_op.drop_index("ix_skills_scope_id")
        for column in (
            "admin_tags",
            "admin_description",
            "active_version_id",
    ...
    with op.batch_alter_table("mcp_servers") as batch_op:
        for column in ("tools", "process_pid", "last_error", "updated_at"):
            batch_op.drop_column(column)
    op.drop_table("core_memory_blocks")
    op.drop_table("audit_logs")
    op.drop_table("archival_passages")
