      ...
      {isMobile ? (
        ...
        <>
          ...
          `} style={{
            ...
            {/* Logo */}
            <div className="h-12 flex items-center px-3" style={{ borderBottom: '1px solid var(--color-border-subtle)' }}>
              {sidebarOpen && (
                <div className="flex items-center gap-2.5">
                  ...
                  <span className="text-sm font-semibold tracking-tight" style={{ color: 'var(--color-text-primary)' }}>Climber</span>
                </div>
              )}
              <button
                ...
                className="ml-auto p-1.5 rounded-lg transition-all duration-200 active:scale-[0.95] hidden md:flex"
              ...
              <button
                onClick={() => setMobileMenuOpen(false)}
                aria-label="关闭导航菜单"
                className="ml-auto p-1.5 rounded-lg transition-all duration-200 active:scale-[0.95] md:hidden"
                style={{ color: 'var(--color-text-muted)' }}
              >
                <X size={14} />
              </button>
            ...
            <div className="p-2.5">
              <button
                ...
                aria-label="全局搜索"
                className="w-full flex items-center gap-2.5 px-2.5 py-2 rounded-lg text-sm transition-all duration-200 active:scale-[0.98]"
                style={{
            ...
            <nav className="flex-1 py-2 px-2 overflow-y-auto">
              <div className="space-y-0.5">
                {CORE_NAV_ITEMS.map(({ id, icon: Icon, label }) => (
                  ...
                  >
                    {currentPage === id && (
                      <div className="absolute left-0 top-1/2 -translate-y-1/2 w-[3px] h-4 rounded-r-full" style={{
                    ...
                    `} style={{
                      backgroundColor: currentPage === id ? 'rgba(99,102,241,0.15)' : 'var(--color-bg-surface-2)',
              ...
              {/* Separator */}
              {sidebarOpen && (
                <div className="my-3 mx-1.5" style={{ borderBottom: '1px solid var(--color-border-subtle)' }} />
              )}

              {/* More pages list (compact) */}
              {sidebarOpen && (
                <div className="space-y-0.5">
                  {ALL_NAV_ITEMS.filter(item => !CORE_NAV_ITEMS.find(core => core.id === item.id)).slice(0, 6).map(({ id, icon: Icon, label }) => (
                    <button
                      key={id}
                      onClick={() => navigate(id)}
            ...
            <div className="p-2.5 space-y-2" style={{ borderTop: '1px solid var(--color-border-subtle)' }}>
              {sidebarOpen && (
                <button
                  ...
                  aria-label="打开命令面板"
                  className="w-full flex items-center gap-2.5 px-2.5 py-2 rounded-lg text-xs transition-all duration-200 active:scale-[0.98]"
                  style={{
                    color: 'var(--color-text-muted)',
                    border: '1px solid var(--color-border-subtle)',
                    backgroundColor: 'var(--color-bg-surface-2)',
