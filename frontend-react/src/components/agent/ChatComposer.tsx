    ...
    <div className="relative mx-auto w-full max-w-4xl px-4 pb-4 pt-2 md:px-6">
      {showCommands && (
        <div className="absolute bottom-full left-6 z-20 mb-2 w-72 overflow-hidden rounded-xl p-1 shadow-xl" style={{ backgroundColor: 'var(--color-bg-surface-1)', border: '1px solid var(--color-border-default)' }} role="listbox" aria-label="斜杠命令">
          <div className="px-2 py-1.5 text-[10px] font-semibold uppercase" style={{ color: 'var(--color-text-muted)' }}>快速指令</div>
          {commands.filter(command => command.name.startsWith(input)).map(command => (
            <button key={command.name} type="button" onClick={() => { setInput(`${command.prompt} `); textareaRef.current?.focus(); requestAnimationFrame(resize); }} className="flex w-full items-center gap-3 rounded-lg px-2.5 py-2 text-left hover:bg-[var(--color-bg-surface-2)]" role="option">
              ...
              <span className="flex-1"><span className="block text-xs font-semibold">{command.name}</span><span className="block text-[10px]" style={{ color: 'var(--color-text-muted)' }}>{command.label}</span></span>
            </button>
          ))}
        </div>
      )}
      ...
      <div className="overflow-hidden rounded-2xl shadow-sm transition-shadow focus-within:shadow-md" style={{ backgroundColor: 'var(--color-bg-surface-1)', border: '1px solid var(--color-border-default)' }}>
        ...
        <div className="flex h-11 items-center gap-1 px-2 pb-2">
          ...
          <span className="ml-auto hidden text-[10px] sm:block" style={{ color: 'var(--color-text-muted)' }}>Enter 发送 · Shift+Enter 换行</span>
          {isLoading ? (
      ...
      <p className="mt-1.5 text-center text-[10px]" style={{ color: 'var(--color-text-muted)' }}>Climber 可能产生不准确结果，执行关键操作前请确认。</p>
    </div>
  );
}
