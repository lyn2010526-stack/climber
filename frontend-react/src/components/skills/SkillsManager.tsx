  ...
  const filtered = useMemo(() => {
    return skills.filter(skill => {
      const matchSearch = !search ||
        ...
        skill.tags?.toLowerCase().includes(search.toLowerCase());
          ...
          <div className="flex items-center justify-between mb-5">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-gray-500 mt-0.5">已启用 {skills.filter(s => s.enabled).length} / {skills.length} 个技能</p>
              </div>
            </div>
            <div className="flex items-center gap-2">
              <button className="p-2 rounded-xl bg-white/[0.04] border border-white/[0.06] text-gray-400 hover:text-white hover:bg-white/[0.08] transition-all">
                <RefreshCw size={14} />
              </button>
          ...
          <div className="flex items-center gap-3">
            <div className="flex-1 relative">
              ...
              <input
                ...
                placeholder="搜索技能..."
                className="w-full h-9 pl-9 pr-4 rounded-xl bg-white/[0.04] border border-white/[0.08] text-sm text-gray-200 placeholder:text-gray-600 focus:outline-none focus:border-blue-500/40 focus:bg-white/[0.06] transition-all"
              />
              {search && (
                <button
                  onClick={() => setSearch('')}
            ...
            <div className="flex items-center bg-white/[0.04] rounded-xl border border-white/[0.06] p-0.5">
              ...
              <button
                ...
                className={cn(
                  'p-1.5 rounded-lg transition-all',
                  viewMode === 'list' ? 'bg-white/[0.08] text-white' : 'text-gray-500 hover:text-gray-300'
                )}
              >
                <List size={14} />
              </button>
        ...
        <div className="flex-1 overflow-y-auto px-6 pb-6">
          {filtered.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-16">
              ...
              <p className="text-xs text-gray-600 mt-1">尝试其他搜索词或分类</p>
            </div>
          ) : viewMode === 'grid' ? (
  ...
  return (
    ...
    >
      <div className={cn(
        ...
        skill.enabled ? 'bg-green-500/10 text-green-400' : 'bg-white/[0.04] text-gray-500'
      )}>
        <Sparkles size={14} />
      </div>
      <div className="flex-1 min-w-0">
        <div className="flex items-center gap-2">
          ...
          <span className="px-1.5 py-0.5 rounded-md text-[10px] bg-white/[0.04] text-gray-500">{skill.category}</span>
        </div>
    ...
    <div className="w-80 border-l border-white/[0.06] bg-[#0D0D12]/80 backdrop-blur-xl flex flex-col">
      <div className="p-5 border-b border-white/[0.06]">
        <div className="flex items-center justify-between mb-4">
          ...
          <button onClick={onClose} className="p-1.5 rounded-lg hover:bg-white/[0.06] text-gray-500 hover:text-gray-300 transition-colors">
            <X size={14} />
          </button>
        </div>
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-[11px] text-gray-500 capitalize">{skill.category}</p>
          </div>
        </div>
      </div>
      ...
      <div className="flex-1 overflow-y-auto p-5 space-y-4">
        <div>
          <label className="text-[10px] font-semibold text-gray-500 uppercase tracking-wider">描述</label>
          <p className="text-xs text-gray-400 mt-1.5 leading-relaxed">{skill.description}</p>
        </div>

        <div>
          <label className="text-[10px] font-semibold text-gray-500 uppercase tracking-wider">标签</label>
          <div className="flex flex-wrap gap-1.5 mt-1.5">
            {skill.tags?.split(',').map(tag => (
              <span key={tag.trim()} className="px-2 py-0.5 rounded-lg text-[10px] bg-white/[0.04] text-gray-400 border border-white/[0.06]">
                {tag.trim()}
              </span>
            ))}
          </div>
        ...
        <div className="grid grid-cols-2 gap-3">
          <div className="p-3 rounded-xl bg-white/[0.03] border border-white/[0.06]">
            <div className="text-[10px] text-gray-500 mb-1">安装量</div>
            <div className="text-sm font-semibold text-white">{skill.installCount?.toLocaleString()}</div>
          </div>
          <div className="p-3 rounded-xl bg-white/[0.03] border border-white/[0.06]">
            <div className="text-[10px] text-gray-500 mb-1">评分</div>
        ...
        <div>
          <label className="text-[10px] font-semibold text-gray-500 uppercase tracking-wider">状态</label>
          <div className="mt-1.5 flex items-center gap-2">
            <div className={`w-2 h-2 rounded-full ${skill.enabled ? 'bg-green-500' : 'bg-gray-600'}`} />
            <span className="text-xs text-gray-400">{skill.enabled ? '已启用' : '已禁用'}</span>
          </div>
        </div>
      </div>
