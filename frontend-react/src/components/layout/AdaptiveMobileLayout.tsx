...
const MOBILE_PRIMARY_IDS: ReadonlySet<string> = new Set(['dashboard', 'chat', 'factory', 'tasks']);
...
export function AdaptiveMobileLayout({ children, currentPage, onNavigate }: {
  ...
  const { primaryItems, moreItems } = useMemo(() => {
    ...
    return {
      ...
      moreItems: ALL_NAV_ITEMS_BASE.filter(item => MOBILE_ADAPTED_PAGE_IDS.has(item.id) && !MOBILE_PRIMARY_IDS.has(item.id)).map(toEntry),
    };
  }, [t]);
