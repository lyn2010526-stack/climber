...
import { fetchFeaturess, createFeatures, updateFeatures, deleteFeatures } from '@/api/features'
...
export const FeaturesPage: React.FC<FeaturesPageProps> = ({ title }) => {
    ...
    const { data, isLoading, error } = useQuery({
        queryKey: ['features', page, pageSize, search, statusFilter, sortBy, sortOrder],
        queryFn: () => fetchFeaturess({ page, pageSize, search, status: statusFilter, sortBy, sortOrder })
    })

    const createMutation = useMutation({
        mutationFn: createFeatures,
        onSuccess: () => {
    ...
    const updateMutation = useMutation({
        mutationFn: (data: Partial<Features>) => updateFeatures(editingId!, data),
        onSuccess: () => {
            queryClient.invalidateQueries({ queryKey: ['features'] })
    ...
    return (
        <div className='space-y-6 p-6'>
            <div className='flex items-center justify-between'>
                <h1 className='text-2xl font-bold'>{title || t('features.title')}</h1>
            ...
            <Modal isOpen={isCreateModalOpen} onClose={() => setIsCreateModalOpen(false)}>
                ...
                <ModalBody>
                    <p>{t('features.create_description')}</p>
                </ModalBody>
                <ModalFooter>
