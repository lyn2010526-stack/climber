  ...
  return (
    <div className="h-full overflow-y-auto p-8">
      <div className="max-w-5xl mx-auto">
        <div className="mb-8">
          <h2 className="text-2xl font-bold text-white">MCP 市场</h2>
        ...
        <div className="flex gap-3 mb-6">
          <div className="flex-1 relative">
            ...
            <input
              ...
              className="w-full pl-10 pr-4 py-2.5 bg-white/5 border border-white/10 rounded-2xl text-sm text-gray-100 placeholder:text-gray-600 focus:outline-none focus:border-[#007AFF]/50 transition-all duration-200"
            />
          </div>
          <select
          ...
          <div key={cat} className="mb-8">
            ...
            <div className="grid grid-cols-2 gap-4">
              {catServers.map(server => (
                <div
                  key={server.id}
                  className={`bg-white/[0.04] border rounded-2xl p-5 transition-all duration-200 backdrop-blur-sm ${
                    server.is_installed ? 'border-green-500/30' : 'border-white/[0.08] hover:border-[#007AFF]/30'
                  }`}
                >
                  <div className="flex items-start gap-3">
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2">
                        <h3 className="font-semibold truncate text-gray-100">{server.name}</h3>
                        {server.is_installed && (
                  ...
                  <div className="mt-3 flex items-center gap-2">
                     <span className="text-xs text-gray-500">作者：{server.author}</span>
                    {server.popularity > 0 && (
                  ...
                  <div className="mt-2 flex flex-wrap gap-1.5">
                    {server.tags.map(tag => (
                      <span key={tag} className="px-2.5 py-1 bg-white/5 text-gray-500 rounded-xl text-xs font-medium border border-white/10">
                        {tag}
                      </span>
                    ))}
                  </div>
                  ...
                  <div className="mt-4 flex justify-end">
                    {server.is_installed ? (
                      <button
                        ...
                        className="flex items-center gap-2 px-4 py-2 border border-red-500/30 text-red-400 hover:bg-red-500/10 rounded-2xl text-sm font-medium transition-all duration-200"
                      >
                        <Trash2 size={14} /> 卸载
                      </button>
                    ) : (
                      <button
                        ...
                        className="flex items-center gap-2 px-4 py-2 bg-[#007AFF] hover:bg-[#007AFF]/90 text-white rounded-2xl text-sm font-semibold disabled:opacity-40 transition-all duration-200 active:scale-[0.97] shadow-lg shadow-blue-500/20"
                      >
                        <Download size={14} />
        ...
        {!loading && !error && filteredServers.length === 0 && (
          <div className="text-center py-16">
            ...
            <p className="text-gray-500 text-sm">未找到 MCP 服务器。</p>
          </div>
        )}
      </div>
    </div>
