...
import { fetchAlertss, createAlerts, updateAlerts, deleteAlerts } from '@/api/alerts'
...
export const AlertsPage: React.FC<AlertsPageProps> = ({ title }) => {
    ...
    const { data, isLoading, error } = useQuery({
        queryKey: ['alerts', page, pageSize, search, statusFilter, sortBy, sortOrder],
        queryFn: () => fetchAlertss({ page, pageSize, search, status: statusFilter, sortBy, sortOrder })
    })

    const createMutation = useMutation({
        mutationFn: createAlerts,
        onSuccess: () => {
    ...
    const updateMutation = useMutation({
        mutationFn: (data: Partial<Alerts>) => updateAlerts(editingId!, data),
        onSuccess: () => {
            queryClient.invalidateQueries({ queryKey: ['alerts'] })
    ...
    return (
        <div className='space-y-6 p-6'>
            <div className='flex items-center justify-between'>
                <h1 className='text-2xl font-bold'>{title || t('alerts.title')}</h1>
            ...
            <Modal isOpen={isCreateModalOpen} onClose={() => setIsCreateModalOpen(false)}>
                ...
                <ModalBody>
                    <p>{t('alerts.create_description')}</p>
                </ModalBody>
                <ModalFooter>
