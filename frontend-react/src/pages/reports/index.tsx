...
import { fetchReportss, createReports, updateReports, deleteReports } from '@/api/reports'
...
export const ReportsPage: React.FC<ReportsPageProps> = ({ title }) => {
    ...
    const { data, isLoading, error } = useQuery({
        queryKey: ['reports', page, pageSize, search, statusFilter, sortBy, sortOrder],
        queryFn: () => fetchReportss({ page, pageSize, search, status: statusFilter, sortBy, sortOrder })
    })
    ...
    const updateMutation = useMutation({
        mutationFn: (data: Partial<Reports>) => updateReports(editingId!, data),
        onSuccess: () => {
            queryClient.invalidateQueries({ queryKey: ['reports'] })
    ...
    return (
        <div className='space-y-6 p-6'>
            <div className='flex items-center justify-between'>
                <h1 className='text-2xl font-bold'>{title || t('reports.title')}</h1>
            ...
            <Modal isOpen={isCreateModalOpen} onClose={() => setIsCreateModalOpen(false)}>
                ...
                <ModalBody>
                    <p>{t('reports.create_description')}</p>
                </ModalBody>
                <ModalFooter>
