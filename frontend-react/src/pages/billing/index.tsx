...
import { fetchBillings, createBilling, updateBilling, deleteBilling } from '@/api/billing'
...
export const BillingPage: React.FC<BillingPageProps> = ({ title }) => {
    ...
    const { data, isLoading, error } = useQuery({
        queryKey: ['billing', page, pageSize, search, statusFilter, sortBy, sortOrder],
        queryFn: () => fetchBillings({ page, pageSize, search, status: statusFilter, sortBy, sortOrder })
    })
    ...
    const updateMutation = useMutation({
        mutationFn: (data: Partial<Billing>) => updateBilling(editingId!, data),
        onSuccess: () => {
            queryClient.invalidateQueries({ queryKey: ['billing'] })
    ...
    return (
        <div className='space-y-6 p-6'>
            <div className='flex items-center justify-between'>
                <h1 className='text-2xl font-bold'>{title || t('billing.title')}</h1>
            ...
            <Modal isOpen={isCreateModalOpen} onClose={() => setIsCreateModalOpen(false)}>
                ...
                <ModalBody>
                    <p>{t('billing.create_description')}</p>
                </ModalBody>
                <ModalFooter>
