...
import { fetchSecuritys, createSecurity, updateSecurity, deleteSecurity } from '@/api/security'
...
export const SecurityPage: React.FC<SecurityPageProps> = ({ title }) => {
    ...
    const { data, isLoading, error } = useQuery({
        queryKey: ['security', page, pageSize, search, statusFilter, sortBy, sortOrder],
        queryFn: () => fetchSecuritys({ page, pageSize, search, status: statusFilter, sortBy, sortOrder })
    })
    ...
    const updateMutation = useMutation({
        mutationFn: (data: Partial<Security>) => updateSecurity(editingId!, data),
        onSuccess: () => {
            queryClient.invalidateQueries({ queryKey: ['security'] })
    ...
    return (
        <div className='space-y-6 p-6'>
            <div className='flex items-center justify-between'>
                <h1 className='text-2xl font-bold'>{title || t('security.title')}</h1>
            ...
            <Modal isOpen={isCreateModalOpen} onClose={() => setIsCreateModalOpen(false)}>
                ...
                <ModalBody>
                    <p>{t('security.create_description')}</p>
                </ModalBody>
                <ModalFooter>
