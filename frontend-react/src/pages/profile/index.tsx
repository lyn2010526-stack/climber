...
import { fetchProfiles, createProfile, updateProfile, deleteProfile } from '@/api/profile'
...
export const ProfilePage: React.FC<ProfilePageProps> = ({ title }) => {
    ...
    const { data, isLoading, error } = useQuery({
        queryKey: ['profile', page, pageSize, search, statusFilter, sortBy, sortOrder],
        queryFn: () => fetchProfiles({ page, pageSize, search, status: statusFilter, sortBy, sortOrder })
    })
    ...
    const updateMutation = useMutation({
        mutationFn: (data: Partial<Profile>) => updateProfile(editingId!, data),
        onSuccess: () => {
            queryClient.invalidateQueries({ queryKey: ['profile'] })
    ...
    return (
        <div className='space-y-6 p-6'>
            <div className='flex items-center justify-between'>
                <h1 className='text-2xl font-bold'>{title || t('profile.title')}</h1>
            ...
            <Modal isOpen={isCreateModalOpen} onClose={() => setIsCreateModalOpen(false)}>
                ...
                <ModalBody>
                    <p>{t('profile.create_description')}</p>
                </ModalBody>
                <ModalFooter>
