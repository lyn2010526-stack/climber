...
import { fetchSubscriptionss, createSubscriptions, updateSubscriptions, deleteSubscriptions } from '@/api/subscriptions'
...
export const SubscriptionsPage: React.FC<SubscriptionsPageProps> = ({ title }) => {
    ...
    const { data, isLoading, error } = useQuery({
        queryKey: ['subscriptions', page, pageSize, search, statusFilter, sortBy, sortOrder],
        queryFn: () => fetchSubscriptionss({ page, pageSize, search, status: statusFilter, sortBy, sortOrder })
    })
    ...
    const updateMutation = useMutation({
        mutationFn: (data: Partial<Subscriptions>) => updateSubscriptions(editingId!, data),
        onSuccess: () => {
            queryClient.invalidateQueries({ queryKey: ['subscriptions'] })
    ...
    const deleteMutation = useMutation({
        mutationFn: deleteSubscriptions,
        onSuccess: () => {
            queryClient.invalidateQueries({ queryKey: ['subscriptions'] })
    ...
    return (
        <div className='space-y-6 p-6'>
            <div className='flex items-center justify-between'>
                <h1 className='text-2xl font-bold'>{title || t('subscriptions.title')}</h1>
            ...
            <Modal isOpen={isCreateModalOpen} onClose={() => setIsCreateModalOpen(false)}>
                ...
                <ModalBody>
                    <p>{t('subscriptions.create_description')}</p>
                </ModalBody>
                <ModalFooter>
