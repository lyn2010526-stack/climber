...
import { fetchPaymentss, createPayments, updatePayments, deletePayments } from '@/api/payments'
...
export const PaymentsPage: React.FC<PaymentsPageProps> = ({ title }) => {
    ...
    const { data, isLoading, error } = useQuery({
        queryKey: ['payments', page, pageSize, search, statusFilter, sortBy, sortOrder],
        queryFn: () => fetchPaymentss({ page, pageSize, search, status: statusFilter, sortBy, sortOrder })
    })
    ...
    const updateMutation = useMutation({
        mutationFn: (data: Partial<Payments>) => updatePayments(editingId!, data),
        onSuccess: () => {
            queryClient.invalidateQueries({ queryKey: ['payments'] })
    ...
    return (
        <div className='space-y-6 p-6'>
            <div className='flex items-center justify-between'>
                <h1 className='text-2xl font-bold'>{title || t('payments.title')}</h1>
            ...
            <Modal isOpen={isCreateModalOpen} onClose={() => setIsCreateModalOpen(false)}>
                ...
                <ModalBody>
                    <p>{t('payments.create_description')}</p>
                </ModalBody>
                <ModalFooter>
