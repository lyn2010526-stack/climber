...
import { fetchUserss, createUsers, updateUsers, deleteUsers } from '@/api/users'
...
export const UsersPage: React.FC<UsersPageProps> = ({ title }) => {
    ...
    const { data, isLoading, error } = useQuery({
        queryKey: ['users', page, pageSize, search, statusFilter, sortBy, sortOrder],
        queryFn: () => fetchUserss({ page, pageSize, search, status: statusFilter, sortBy, sortOrder })
    })
    ...
    const updateMutation = useMutation({
        mutationFn: (data: Partial<Users>) => updateUsers(editingId!, data),
        onSuccess: () => {
            queryClient.invalidateQueries({ queryKey: ['users'] })
    ...
    return (
        <div className='space-y-6 p-6'>
            <div className='flex items-center justify-between'>
                <h1 className='text-2xl font-bold'>{title || t('users.title')}</h1>
            ...
            <Modal isOpen={isCreateModalOpen} onClose={() => setIsCreateModalOpen(false)}>
                ...
                <ModalBody>
                    <p>{t('users.create_description')}</p>
                </ModalBody>
                <ModalFooter>
