...
import { fetchReleasess, createReleases, updateReleases, deleteReleases } from '@/api/releases'
...
export const ReleasesPage: React.FC<ReleasesPageProps> = ({ title }) => {
    ...
    const { data, isLoading, error } = useQuery({
        queryKey: ['releases', page, pageSize, search, statusFilter, sortBy, sortOrder],
        queryFn: () => fetchReleasess({ page, pageSize, search, status: statusFilter, sortBy, sortOrder })
    })

    const createMutation = useMutation({
        mutationFn: createReleases,
        onSuccess: () => {
    ...
    const updateMutation = useMutation({
        mutationFn: (data: Partial<Releases>) => updateReleases(editingId!, data),
        onSuccess: () => {
            queryClient.invalidateQueries({ queryKey: ['releases'] })
    ...
    return (
        <div className='space-y-6 p-6'>
            <div className='flex items-center justify-between'>
                <h1 className='text-2xl font-bold'>{title || t('releases.title')}</h1>
            ...
            <Modal isOpen={isCreateModalOpen} onClose={() => setIsCreateModalOpen(false)}>
                ...
                <ModalBody>
                    <p>{t('releases.create_description')}</p>
                </ModalBody>
                <ModalFooter>
