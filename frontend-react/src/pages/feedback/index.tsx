...
import { fetchFeedbacks, createFeedback, updateFeedback, deleteFeedback } from '@/api/feedback'
...
export const FeedbackPage: React.FC<FeedbackPageProps> = ({ title }) => {
    ...
    const { data, isLoading, error } = useQuery({
        queryKey: ['feedback', page, pageSize, search, statusFilter, sortBy, sortOrder],
        queryFn: () => fetchFeedbacks({ page, pageSize, search, status: statusFilter, sortBy, sortOrder })
    })
    ...
    const updateMutation = useMutation({
        mutationFn: (data: Partial<Feedback>) => updateFeedback(editingId!, data),
        onSuccess: () => {
            queryClient.invalidateQueries({ queryKey: ['feedback'] })
    ...
    return (
        <div className='space-y-6 p-6'>
            <div className='flex items-center justify-between'>
                <h1 className='text-2xl font-bold'>{title || t('feedback.title')}</h1>
            ...
            <Modal isOpen={isCreateModalOpen} onClose={() => setIsCreateModalOpen(false)}>
                ...
                <ModalBody>
                    <p>{t('feedback.create_description')}</p>
                </ModalBody>
                <ModalFooter>
