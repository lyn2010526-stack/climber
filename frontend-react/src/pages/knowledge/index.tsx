...
import { fetchKnowledges, createKnowledge, updateKnowledge, deleteKnowledge } from '@/api/knowledge'
...
export const KnowledgePage: React.FC<KnowledgePageProps> = ({ title }) => {
    ...
    const { data, isLoading, error } = useQuery({
        queryKey: ['knowledge', page, pageSize, search, statusFilter, sortBy, sortOrder],
        queryFn: () => fetchKnowledges({ page, pageSize, search, status: statusFilter, sortBy, sortOrder })
    })
    ...
    const updateMutation = useMutation({
        mutationFn: (data: Partial<Knowledge>) => updateKnowledge(editingId!, data),
        onSuccess: () => {
            queryClient.invalidateQueries({ queryKey: ['knowledge'] })
    ...
    return (
        <div className='space-y-6 p-6'>
            <div className='flex items-center justify-between'>
                <h1 className='text-2xl font-bold'>{title || t('knowledge.title')}</h1>
            ...
            <Modal isOpen={isCreateModalOpen} onClose={() => setIsCreateModalOpen(false)}>
                ...
                <ModalBody>
                    <p>{t('knowledge.create_description')}</p>
                </ModalBody>
                <ModalFooter>
