...
import { fetchIncidentss, createIncidents, updateIncidents, deleteIncidents } from '@/api/incidents'
...
export const IncidentsPage: React.FC<IncidentsPageProps> = ({ title }) => {
    ...
    const { data, isLoading, error } = useQuery({
        queryKey: ['incidents', page, pageSize, search, statusFilter, sortBy, sortOrder],
        queryFn: () => fetchIncidentss({ page, pageSize, search, status: statusFilter, sortBy, sortOrder })
    })
    ...
    const updateMutation = useMutation({
        mutationFn: (data: Partial<Incidents>) => updateIncidents(editingId!, data),
        onSuccess: () => {
            queryClient.invalidateQueries({ queryKey: ['incidents'] })
    ...
    return (
        <div className='space-y-6 p-6'>
            <div className='flex items-center justify-between'>
                <h1 className='text-2xl font-bold'>{title || t('incidents.title')}</h1>
            ...
            <Modal isOpen={isCreateModalOpen} onClose={() => setIsCreateModalOpen(false)}>
                ...
                <ModalBody>
                    <p>{t('incidents.create_description')}</p>
                </ModalBody>
                <ModalFooter>
