...
import { fetchOnboardings, createOnboarding, updateOnboarding, deleteOnboarding } from '@/api/onboarding'
...
export const OnboardingPage: React.FC<OnboardingPageProps> = ({ title }) => {
    ...
    const { data, isLoading, error } = useQuery({
        queryKey: ['onboarding', page, pageSize, search, statusFilter, sortBy, sortOrder],
        queryFn: () => fetchOnboardings({ page, pageSize, search, status: statusFilter, sortBy, sortOrder })
    })
    ...
    const updateMutation = useMutation({
        mutationFn: (data: Partial<Onboarding>) => updateOnboarding(editingId!, data),
        onSuccess: () => {
            queryClient.invalidateQueries({ queryKey: ['onboarding'] })
    ...
    return (
        <div className='space-y-6 p-6'>
            <div className='flex items-center justify-between'>
                <h1 className='text-2xl font-bold'>{title || t('onboarding.title')}</h1>
            ...
            <Modal isOpen={isCreateModalOpen} onClose={() => setIsCreateModalOpen(false)}>
                ...
                <ModalBody>
                    <p>{t('onboarding.create_description')}</p>
                </ModalBody>
                <ModalFooter>
