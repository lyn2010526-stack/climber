...
import { fetchTeamss, createTeams, updateTeams, deleteTeams } from '@/api/teams'
...
export const TeamsPage: React.FC<TeamsPageProps> = ({ title }) => {
    ...
    const { data, isLoading, error } = useQuery({
        queryKey: ['teams', page, pageSize, search, statusFilter, sortBy, sortOrder],
        queryFn: () => fetchTeamss({ page, pageSize, search, status: statusFilter, sortBy, sortOrder })
    })
    ...
    const updateMutation = useMutation({
        mutationFn: (data: Partial<Teams>) => updateTeams(editingId!, data),
        onSuccess: () => {
            queryClient.invalidateQueries({ queryKey: ['teams'] })
    ...
    return (
        <div className='space-y-6 p-6'>
            <div className='flex items-center justify-between'>
                <h1 className='text-2xl font-bold'>{title || t('teams.title')}</h1>
            ...
            <Modal isOpen={isCreateModalOpen} onClose={() => setIsCreateModalOpen(false)}>
                ...
                <ModalBody>
                    <p>{t('teams.create_description')}</p>
                </ModalBody>
                <ModalFooter>
