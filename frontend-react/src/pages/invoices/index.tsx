...
import { fetchInvoicess, createInvoices, updateInvoices, deleteInvoices } from '@/api/invoices'
...
export const InvoicesPage: React.FC<InvoicesPageProps> = ({ title }) => {
    ...
    const { data, isLoading, error } = useQuery({
        queryKey: ['invoices', page, pageSize, search, statusFilter, sortBy, sortOrder],
        queryFn: () => fetchInvoicess({ page, pageSize, search, status: statusFilter, sortBy, sortOrder })
    })
    ...
    const updateMutation = useMutation({
        mutationFn: (data: Partial<Invoices>) => updateInvoices(editingId!, data),
        onSuccess: () => {
            queryClient.invalidateQueries({ queryKey: ['invoices'] })
    ...
    return (
        <div className='space-y-6 p-6'>
            <div className='flex items-center justify-between'>
                <h1 className='text-2xl font-bold'>{title || t('invoices.title')}</h1>
            ...
            <Modal isOpen={isCreateModalOpen} onClose={() => setIsCreateModalOpen(false)}>
                ...
                <ModalBody>
                    <p>{t('invoices.create_description')}</p>
                </ModalBody>
                <ModalFooter>
