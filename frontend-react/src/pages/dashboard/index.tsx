...
import { fetchDashboards, createDashboard, updateDashboard, deleteDashboard } from '@/api/dashboard'
...
export const DashboardPage: React.FC<DashboardPageProps> = ({ title }) => {
    ...
    const { data, isLoading, error } = useQuery({
        queryKey: ['dashboard', page, pageSize, search, statusFilter, sortBy, sortOrder],
        queryFn: () => fetchDashboards({ page, pageSize, search, status: statusFilter, sortBy, sortOrder })
    })
    ...
    const updateMutation = useMutation({
        mutationFn: (data: Partial<Dashboard>) => updateDashboard(editingId!, data),
        onSuccess: () => {
            queryClient.invalidateQueries({ queryKey: ['dashboard'] })
    ...
    return (
        <div className='space-y-6 p-6'>
            <div className='flex items-center justify-between'>
                <h1 className='text-2xl font-bold'>{title || t('dashboard.title')}</h1>
            ...
            <Modal isOpen={isCreateModalOpen} onClose={() => setIsCreateModalOpen(false)}>
                ...
                <ModalBody>
                    <p>{t('dashboard.create_description')}</p>
                </ModalBody>
                <ModalFooter>
