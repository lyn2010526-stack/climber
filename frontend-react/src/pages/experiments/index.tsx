...
import { fetchExperimentss, createExperiments, updateExperiments, deleteExperiments } from '@/api/experiments'
...
export const ExperimentsPage: React.FC<ExperimentsPageProps> = ({ title }) => {
    ...
    const { data, isLoading, error } = useQuery({
        queryKey: ['experiments', page, pageSize, search, statusFilter, sortBy, sortOrder],
        queryFn: () => fetchExperimentss({ page, pageSize, search, status: statusFilter, sortBy, sortOrder })
    })

    const createMutation = useMutation({
        mutationFn: createExperiments,
        onSuccess: () => {
    ...
    const updateMutation = useMutation({
        mutationFn: (data: Partial<Experiments>) => updateExperiments(editingId!, data),
        onSuccess: () => {
            queryClient.invalidateQueries({ queryKey: ['experiments'] })
    ...
    return (
        <div className='space-y-6 p-6'>
            <div className='flex items-center justify-between'>
                <h1 className='text-2xl font-bold'>{title || t('experiments.title')}</h1>
            ...
            <Modal isOpen={isCreateModalOpen} onClose={() => setIsCreateModalOpen(false)}>
                ...
                <ModalBody>
                    <p>{t('experiments.create_description')}</p>
                </ModalBody>
                <ModalFooter>
