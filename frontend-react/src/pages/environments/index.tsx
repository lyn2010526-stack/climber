...
import { fetchEnvironmentss, createEnvironments, updateEnvironments, deleteEnvironments } from '@/api/environments'
...
export const EnvironmentsPage: React.FC<EnvironmentsPageProps> = ({ title }) => {
    ...
    const { data, isLoading, error } = useQuery({
        queryKey: ['environments', page, pageSize, search, statusFilter, sortBy, sortOrder],
        queryFn: () => fetchEnvironmentss({ page, pageSize, search, status: statusFilter, sortBy, sortOrder })
    })

    const createMutation = useMutation({
        mutationFn: createEnvironments,
        onSuccess: () => {
    ...
    const updateMutation = useMutation({
        mutationFn: (data: Partial<Environments>) => updateEnvironments(editingId!, data),
        onSuccess: () => {
            queryClient.invalidateQueries({ queryKey: ['environments'] })
    ...
    return (
        <div className='space-y-6 p-6'>
            <div className='flex items-center justify-between'>
                <h1 className='text-2xl font-bold'>{title || t('environments.title')}</h1>
            ...
            <Modal isOpen={isCreateModalOpen} onClose={() => setIsCreateModalOpen(false)}>
                ...
                <ModalBody>
                    <p>{t('environments.create_description')}</p>
                </ModalBody>
                <ModalFooter>
