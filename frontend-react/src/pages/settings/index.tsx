...
import { fetchSettingss, createSettings, updateSettings, deleteSettings } from '@/api/settings'
...
export const SettingsPage: React.FC<SettingsPageProps> = ({ title }) => {
    ...
    const { data, isLoading, error } = useQuery({
        queryKey: ['settings', page, pageSize, search, statusFilter, sortBy, sortOrder],
        queryFn: () => fetchSettingss({ page, pageSize, search, status: statusFilter, sortBy, sortOrder })
    })
    ...
    const updateMutation = useMutation({
        mutationFn: (data: Partial<Settings>) => updateSettings(editingId!, data),
        onSuccess: () => {
            queryClient.invalidateQueries({ queryKey: ['settings'] })
    ...
    return (
        <div className='space-y-6 p-6'>
            <div className='flex items-center justify-between'>
                <h1 className='text-2xl font-bold'>{title || t('settings.title')}</h1>
            ...
            <Modal isOpen={isCreateModalOpen} onClose={() => setIsCreateModalOpen(false)}>
                ...
                <ModalBody>
                    <p>{t('settings.create_description')}</p>
                </ModalBody>
                <ModalFooter>
