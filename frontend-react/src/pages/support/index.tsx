...
import { fetchSupports, createSupport, updateSupport, deleteSupport } from '@/api/support'
...
export const SupportPage: React.FC<SupportPageProps> = ({ title }) => {
    ...
    const { data, isLoading, error } = useQuery({
        queryKey: ['support', page, pageSize, search, statusFilter, sortBy, sortOrder],
        queryFn: () => fetchSupports({ page, pageSize, search, status: statusFilter, sortBy, sortOrder })
    })
    ...
    const updateMutation = useMutation({
        mutationFn: (data: Partial<Support>) => updateSupport(editingId!, data),
        onSuccess: () => {
            queryClient.invalidateQueries({ queryKey: ['support'] })
    ...
    return (
        <div className='space-y-6 p-6'>
            <div className='flex items-center justify-between'>
                <h1 className='text-2xl font-bold'>{title || t('support.title')}</h1>
            ...
            <Modal isOpen={isCreateModalOpen} onClose={() => setIsCreateModalOpen(false)}>
                ...
                <ModalBody>
                    <p>{t('support.create_description')}</p>
                </ModalBody>
                <ModalFooter>
