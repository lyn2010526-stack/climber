...
import { fetchAnalyticss, createAnalytics, updateAnalytics, deleteAnalytics } from '@/api/analytics'
...
export const AnalyticsPage: React.FC<AnalyticsPageProps> = ({ title }) => {
    ...
    const { data, isLoading, error } = useQuery({
        queryKey: ['analytics', page, pageSize, search, statusFilter, sortBy, sortOrder],
        queryFn: () => fetchAnalyticss({ page, pageSize, search, status: statusFilter, sortBy, sortOrder })
    })
    ...
    const updateMutation = useMutation({
        mutationFn: (data: Partial<Analytics>) => updateAnalytics(editingId!, data),
        onSuccess: () => {
            queryClient.invalidateQueries({ queryKey: ['analytics'] })
    ...
    return (
        <div className='space-y-6 p-6'>
            <div className='flex items-center justify-between'>
                <h1 className='text-2xl font-bold'>{title || t('analytics.title')}</h1>
            ...
            <Modal isOpen={isCreateModalOpen} onClose={() => setIsCreateModalOpen(false)}>
                ...
                <ModalBody>
                    <p>{t('analytics.create_description')}</p>
                </ModalBody>
                <ModalFooter>
