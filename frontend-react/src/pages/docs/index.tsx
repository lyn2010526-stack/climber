...
import { fetchDocss, createDocs, updateDocs, deleteDocs } from '@/api/docs'
...
export const DocsPage: React.FC<DocsPageProps> = ({ title }) => {
    ...
    const { data, isLoading, error } = useQuery({
        queryKey: ['docs', page, pageSize, search, statusFilter, sortBy, sortOrder],
        queryFn: () => fetchDocss({ page, pageSize, search, status: statusFilter, sortBy, sortOrder })
    })
    ...
    const updateMutation = useMutation({
        mutationFn: (data: Partial<Docs>) => updateDocs(editingId!, data),
        onSuccess: () => {
            queryClient.invalidateQueries({ queryKey: ['docs'] })
    ...
    return (
        <div className='space-y-6 p-6'>
            <div className='flex items-center justify-between'>
                <h1 className='text-2xl font-bold'>{title || t('docs.title')}</h1>
