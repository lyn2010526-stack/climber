...
import { fetchAudits, createAudit, updateAudit, deleteAudit } from '@/api/audit'
...
export const AuditPage: React.FC<AuditPageProps> = ({ title }) => {
    ...
    const { data, isLoading, error } = useQuery({
        queryKey: ['audit', page, pageSize, search, statusFilter, sortBy, sortOrder],
        queryFn: () => fetchAudits({ page, pageSize, search, status: statusFilter, sortBy, sortOrder })
    })
    ...
    const updateMutation = useMutation({
        mutationFn: (data: Partial<Audit>) => updateAudit(editingId!, data),
        onSuccess: () => {
            queryClient.invalidateQueries({ queryKey: ['audit'] })
    ...
    return (
        <div className='space-y-6 p-6'>
            <div className='flex items-center justify-between'>
                <h1 className='text-2xl font-bold'>{title || t('audit.title')}</h1>
            ...
            <Modal isOpen={isCreateModalOpen} onClose={() => setIsCreateModalOpen(false)}>
                ...
                <ModalBody>
                    <p>{t('audit.create_description')}</p>
                </ModalBody>
                <ModalFooter>
