...
type KpisHookReturn = KpisState & KpisActions


export function useKpis(initialFilters?: Record<string, any>): KpisHookReturn {
    const { showToast } = useToast()
    const { user } = useAuth()
    const queryClient = useQueryClient()
    const abortControllerRef = useRef<AbortController | null>(null)

    const [state, setState] = useState<KpisState>({
        data: null,
        loading: false,
        error: null,
        page: 1,
    ...
    const { data, isLoading, error, refetch } = useQuery({
        queryKey: ['kpis', state.page, state.pageSize, state.filters, state.sortBy, state.sortOrder],
        queryFn: async () => {
