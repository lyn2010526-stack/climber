...
type VersionsHookReturn = VersionsState & VersionsActions


export function useVersions(initialFilters?: Record<string, any>): VersionsHookReturn {
    ...
    const { data, isLoading, error, refetch } = useQuery({
        queryKey: ['versions', state.page, state.pageSize, state.filters, state.sortBy, state.sortOrder],
        queryFn: async () => {
    ...
    const updateMutation = useMutation({
        mutationFn: async ({ id, data }: { id: number; data: any }) => {
            ...
            const response = await apiClient.put('/versions/' + id, data)
            return response.data
        },
        onSuccess: () => {
            queryClient.invalidateQueries({ queryKey: ['versions'] })
