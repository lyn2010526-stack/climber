...
describe('useScroll', () => {
  ...
  it('returns initial loading state when immediate is true', () => {
    ...
    const { result } = renderHook(() => useScroll('test-key', fetcher, { immediate: true }));
  ...
  it('does not fetch when immediate is false', () => {
    const fetcher = vi.fn().mockResolvedValue('data');
    const { result } = renderHook(() => useScroll('test-key', fetcher, { immediate: false }));
  ...
  it('does not fetch when disabled', () => {
    const fetcher = vi.fn().mockResolvedValue('data');
    renderHook(() => useScroll('test-key', fetcher, { enabled: false, immediate: true }));
    expect(fetcher).not.toHaveBeenCalled();
  });
});
