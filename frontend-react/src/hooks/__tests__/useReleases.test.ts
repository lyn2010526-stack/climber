...
describe('useReleases', () => {
  ...
  it('create posts to /releases', async () => {
    const { apiClient } = await import('@/lib/api-client');
    ...
    expect(apiClient.post).toHaveBeenCalledWith('/releases', { name: 'New Item' });
  });

  it('update puts to /releases/{id}', async () => {
    const { apiClient } = await import('@/lib/api-client');
    ...
    expect(apiClient.put).toHaveBeenCalledWith('/releases/1', { name: 'Updated' });
  });

  it('remove deletes /releases/{id}', async () => {
    const { apiClient } = await import('@/lib/api-client');
    ...
    expect(apiClient.delete).toHaveBeenCalledWith('/releases/1');
  });
