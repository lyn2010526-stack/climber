...
describe('useMetrics', () => {
  ...
  it('create posts to /metrics', async () => {
    const { apiClient } = await import('@/lib/api-client');
    ...
    expect(apiClient.post).toHaveBeenCalledWith('/metrics', { name: 'New Item' });
  });

  it('update puts to /metrics/{id}', async () => {
    const { apiClient } = await import('@/lib/api-client');
    ...
    expect(apiClient.put).toHaveBeenCalledWith('/metrics/1', { name: 'Updated' });
  });

  it('remove deletes /metrics/{id}', async () => {
    const { apiClient } = await import('@/lib/api-client');
    ...
    expect(apiClient.delete).toHaveBeenCalledWith('/metrics/1');
  });
