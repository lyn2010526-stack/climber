...
describe('useKpis', () => {
  ...
  it('create posts to /kpis', async () => {
    ...
    expect(apiClient.post).toHaveBeenCalledWith('/kpis', { name: 'New Item' });
  });

  it('update puts to /kpis/{id}', async () => {
    const { apiClient } = await import('@/lib/api-client');
    ...
    expect(apiClient.put).toHaveBeenCalledWith('/kpis/1', { name: 'Updated' });
  });

  it('remove deletes /kpis/{id}', async () => {
    const { apiClient } = await import('@/lib/api-client');
    ...
    expect(apiClient.delete).toHaveBeenCalledWith('/kpis/1');
  });
