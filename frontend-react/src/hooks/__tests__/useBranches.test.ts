...
describe('useBranches', () => {
  ...
  it('create posts to /branches', async () => {
    const { apiClient } = await import('@/lib/api-client');
    ...
    expect(apiClient.post).toHaveBeenCalledWith('/branches', { name: 'New Item' });
  });

  it('update puts to /branches/{id}', async () => {
    const { apiClient } = await import('@/lib/api-client');
    ...
    expect(apiClient.put).toHaveBeenCalledWith('/branches/1', { name: 'Updated' });
  });
