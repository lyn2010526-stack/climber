...
describe('useActions', () => {
  ...
  it('create posts to /actions', async () => {
    const { apiClient } = await import('@/lib/api-client');
    ...
    expect(apiClient.post).toHaveBeenCalledWith('/actions', { name: 'New Item' });
  });

  it('update puts to /actions/{id}', async () => {
    const { apiClient } = await import('@/lib/api-client');
    ...
    expect(apiClient.put).toHaveBeenCalledWith('/actions/1', { name: 'Updated' });
  });

  it('remove deletes /actions/{id}', async () => {
    const { apiClient } = await import('@/lib/api-client');
    ...
    expect(apiClient.delete).toHaveBeenCalledWith('/actions/1');
  });
