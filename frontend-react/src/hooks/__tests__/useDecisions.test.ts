...
describe('useDecisions', () => {
  ...
  it('create posts to /decisions', async () => {
    const { apiClient } = await import('@/lib/api-client');
    ...
    expect(apiClient.post).toHaveBeenCalledWith('/decisions', { name: 'New Item' });
  });

  it('update puts to /decisions/{id}', async () => {
    const { apiClient } = await import('@/lib/api-client');
    ...
    expect(apiClient.put).toHaveBeenCalledWith('/decisions/1', { name: 'Updated' });
  });

  it('remove deletes /decisions/{id}', async () => {
    const { apiClient } = await import('@/lib/api-client');
    ...
    expect(apiClient.delete).toHaveBeenCalledWith('/decisions/1');
  });
