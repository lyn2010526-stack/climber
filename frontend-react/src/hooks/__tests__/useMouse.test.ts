...
describe('useMouse', () => {
  ...
  it('returns initial loading state when immediate is true', () => {
    ...
    const { result } = renderHook(() => useMouse('test-key', fetcher, { immediate: true }));
  ...
  it('does not fetch when immediate is false', () => {
    const fetcher = vi.fn().mockResolvedValue('data');
    const { result } = renderHook(() => useMouse('test-key', fetcher, { immediate: false }));
  ...
  it('does not fetch when disabled', () => {
    const fetcher = vi.fn().mockResolvedValue('data');
    renderHook(() => useMouse('test-key', fetcher, { enabled: false, immediate: true }));
    expect(fetcher).not.toHaveBeenCalled();
  });
});
