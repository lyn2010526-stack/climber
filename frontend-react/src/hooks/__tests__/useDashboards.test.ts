...
describe('useDashboards', () => {
  ...
  it('create posts to /dashboards', async () => {
    const { apiClient } = await import('@/lib/api-client');
    ...
    expect(apiClient.post).toHaveBeenCalledWith('/dashboards', { name: 'New Item' });
  });

  it('update puts to /dashboards/{id}', async () => {
    const { apiClient } = await import('@/lib/api-client');
    ...
    expect(apiClient.put).toHaveBeenCalledWith('/dashboards/1', { name: 'Updated' });
  });

  it('remove deletes /dashboards/{id}', async () => {
    const { apiClient } = await import('@/lib/api-client');
