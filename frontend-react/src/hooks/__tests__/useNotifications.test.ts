...
describe('useNotifications', () => {
  ...
  it('create posts to /notifications', async () => {
    const { apiClient } = await import('@/lib/api-client');
    ...
    expect(apiClient.post).toHaveBeenCalledWith('/notifications', { name: 'New Item' });
  });

  it('update puts to /notifications/{id}', async () => {
    const { apiClient } = await import('@/lib/api-client');
    ...
    expect(apiClient.put).toHaveBeenCalledWith('/notifications/1', { name: 'Updated' });
  });
