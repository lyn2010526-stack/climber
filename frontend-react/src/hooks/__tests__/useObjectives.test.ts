...
describe('useObjectives', () => {
  ...
  it('create posts to /objectives', async () => {
    const { apiClient } = await import('@/lib/api-client');
    ...
    expect(apiClient.post).toHaveBeenCalledWith('/objectives', { name: 'New Item' });
  });

  it('update puts to /objectives/{id}', async () => {
    const { apiClient } = await import('@/lib/api-client');
    ...
    expect(apiClient.put).toHaveBeenCalledWith('/objectives/1', { name: 'Updated' });
  });

  it('remove deletes /objectives/{id}', async () => {
    const { apiClient } = await import('@/lib/api-client');
    ...
    expect(apiClient.delete).toHaveBeenCalledWith('/objectives/1');
  });
