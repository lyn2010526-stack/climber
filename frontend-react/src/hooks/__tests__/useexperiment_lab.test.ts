...
import { useExperimentLab} from '../useexperiment_lab';

describe('useexperiment_lab', () => {
  beforeEach(() => {
    vi.useRealTimers();
  });
  ...
  it('returns initial loading state when autoFetch is true', () => {
    ...
    const { result } = renderHook(() => useExperimentLab('test-key', fetcher, { autoFetch: true }));
  ...
  it('does not fetch when autoFetch is false', () => {
    const fetcher = vi.fn().mockResolvedValue('data');
    const { result } = renderHook(() => useExperimentLab('test-key', fetcher, { autoFetch: false }));
  ...
  it('does not fetch when autoFetch is disabled', () => {
    const fetcher = vi.fn().mockResolvedValue('data');
    renderHook(() => useExperimentLab('test-key', fetcher, { autoFetch: false }));
    expect(fetcher).not.toHaveBeenCalled();
  });
});
