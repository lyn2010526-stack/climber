...
describe('useDependencies', () => {
  ...
  it('create posts to /dependencies', async () => {
    const { apiClient } = await import('@/lib/api-client');
    ...
    expect(apiClient.post).toHaveBeenCalledWith('/dependencies', { name: 'New Item' });
  });

  it('update puts to /dependencies/{id}', async () => {
    const { apiClient } = await import('@/lib/api-client');
    ...
    expect(apiClient.put).toHaveBeenCalledWith('/dependencies/1', { name: 'Updated' });
  });

  it('remove deletes /dependencies/{id}', async () => {
    const { apiClient } = await import('@/lib/api-client');
    ...
    expect(apiClient.delete).toHaveBeenCalledWith('/dependencies/1');
  });
