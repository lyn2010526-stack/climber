...
describe('useGoals', () => {
  ...
  it('create posts to /goals', async () => {
    const { apiClient } = await import('@/lib/api-client');
    ...
    expect(apiClient.post).toHaveBeenCalledWith('/goals', { name: 'New Item' });
  });

  it('update puts to /goals/{id}', async () => {
    const { apiClient } = await import('@/lib/api-client');
    ...
    expect(apiClient.put).toHaveBeenCalledWith('/goals/1', { name: 'Updated' });
  });

  it('remove deletes /goals/{id}', async () => {
    const { apiClient } = await import('@/lib/api-client');
