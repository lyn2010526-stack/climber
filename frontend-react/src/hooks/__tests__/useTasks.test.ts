...
describe('useTasks', () => {
  ...
  it('create posts to /tasks', async () => {
    ...
    expect(apiClient.post).toHaveBeenCalledWith('/tasks', { name: 'New Item' });
  });

  it('update puts to /tasks/{id}', async () => {
    const { apiClient } = await import('@/lib/api-client');
    ...
    expect(apiClient.put).toHaveBeenCalledWith('/tasks/1', { name: 'Updated' });
  });

  it('remove deletes /tasks/{id}', async () => {
    const { apiClient } = await import('@/lib/api-client');
