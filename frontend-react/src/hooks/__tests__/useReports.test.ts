...
describe('useReports', () => {
  ...
  it('create posts to /reports', async () => {
    ...
    expect(apiClient.post).toHaveBeenCalledWith('/reports', { name: 'New Item' });
  });

  it('update puts to /reports/{id}', async () => {
    const { apiClient } = await import('@/lib/api-client');
    ...
    expect(apiClient.put).toHaveBeenCalledWith('/reports/1', { name: 'Updated' });
  });

  it('remove deletes /reports/{id}', async () => {
    const { apiClient } = await import('@/lib/api-client');
