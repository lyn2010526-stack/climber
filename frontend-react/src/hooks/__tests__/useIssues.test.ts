...
describe('useIssues', () => {
  ...
  it('create posts to /issues', async () => {
    const { apiClient } = await import('@/lib/api-client');
    ...
    expect(apiClient.post).toHaveBeenCalledWith('/issues', { name: 'New Item' });
  });

  it('update puts to /issues/{id}', async () => {
    const { apiClient } = await import('@/lib/api-client');
    ...
    expect(apiClient.put).toHaveBeenCalledWith('/issues/1', { name: 'Updated' });
  });

  it('remove deletes /issues/{id}', async () => {
    const { apiClient } = await import('@/lib/api-client');
    ...
    expect(apiClient.delete).toHaveBeenCalledWith('/issues/1');
  });
