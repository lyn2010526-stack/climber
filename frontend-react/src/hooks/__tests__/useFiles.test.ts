...
describe('useFiles', () => {
  ...
  it('create posts to /files', async () => {
    const { apiClient } = await import('@/lib/api-client');
    ...
    expect(apiClient.post).toHaveBeenCalledWith('/files', { name: 'New Item' });
  });

  it('update puts to /files/{id}', async () => {
    const { apiClient } = await import('@/lib/api-client');
    ...
    expect(apiClient.put).toHaveBeenCalledWith('/files/1', { name: 'Updated' });
  });

  it('remove deletes /files/{id}', async () => {
    const { apiClient } = await import('@/lib/api-client');
    ...
    expect(apiClient.delete).toHaveBeenCalledWith('/files/1');
  });
