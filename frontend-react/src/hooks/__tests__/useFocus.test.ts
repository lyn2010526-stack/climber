...
describe('useFocus', () => {
  ...
  it('returns initial loading state when immediate is true', () => {
    ...
    const { result } = renderHook(() => useFocus('test-key', fetcher, { immediate: true }));
  ...
  it('does not fetch when immediate is false', () => {
    const fetcher = vi.fn().mockResolvedValue('data');
    const { result } = renderHook(() => useFocus('test-key', fetcher, { immediate: false }));
  ...
  it('does not fetch when disabled', () => {
    const fetcher = vi.fn().mockResolvedValue('data');
    renderHook(() => useFocus('test-key', fetcher, { enabled: false, immediate: true }));
    expect(fetcher).not.toHaveBeenCalled();
  });
});
