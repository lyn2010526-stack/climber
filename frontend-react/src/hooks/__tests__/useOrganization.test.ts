...
describe('useOrganization', () => {
  ...
  it('returns initial loading state when autoFetch is true', () => {
    ...
    const { result } = renderHook(() => useOrganization('test-key', fetcher, { autoFetch: true }));
  ...
  it('does not fetch when autoFetch is false', () => {
    const fetcher = vi.fn().mockResolvedValue('data');
    const { result } = renderHook(() => useOrganization('test-key', fetcher, { autoFetch: false }));
  ...
  it('setData sets data directly', () => {
    const fetcher = vi.fn().mockResolvedValue('data');
    const { result } = renderHook(() => useOrganization('test-key', fetcher, { autoFetch: false }));
    act(() => {
      result.current.setData('mutated-data');
    });
  ...
  it('does not fetch when autoFetch is disabled', () => {
    const fetcher = vi.fn().mockResolvedValue('data');
    renderHook(() => useOrganization('test-key', fetcher, { autoFetch: false }));
    expect(fetcher).not.toHaveBeenCalled();
  });
});
