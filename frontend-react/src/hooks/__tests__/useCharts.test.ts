...
describe('useCharts', () => {
  ...
  it('create posts to /charts', async () => {
    const { apiClient } = await import('@/lib/api-client');
    ...
    expect(apiClient.post).toHaveBeenCalledWith('/charts', { name: 'New Item' });
  });

  it('update puts to /charts/{id}', async () => {
    const { apiClient } = await import('@/lib/api-client');
    ...
    expect(apiClient.put).toHaveBeenCalledWith('/charts/1', { name: 'Updated' });
  });

  it('remove deletes /charts/{id}', async () => {
    const { apiClient } = await import('@/lib/api-client');
    ...
    expect(apiClient.delete).toHaveBeenCalledWith('/charts/1');
  });
