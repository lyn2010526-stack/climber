...
describe('useWidgets', () => {
  ...
  it('create posts to /widgets', async () => {
    const { apiClient } = await import('@/lib/api-client');
    ...
    expect(apiClient.post).toHaveBeenCalledWith('/widgets', { name: 'New Item' });
  });

  it('update puts to /widgets/{id}', async () => {
    const { apiClient } = await import('@/lib/api-client');
    ...
    expect(apiClient.put).toHaveBeenCalledWith('/widgets/1', { name: 'Updated' });
  });

  it('remove deletes /widgets/{id}', async () => {
    const { apiClient } = await import('@/lib/api-client');
    ...
    expect(apiClient.delete).toHaveBeenCalledWith('/widgets/1');
  });
