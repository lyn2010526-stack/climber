...
describe('useComments', () => {
  ...
  it('create posts to /comments', async () => {
    const { apiClient } = await import('@/lib/api-client');
    ...
    expect(apiClient.post).toHaveBeenCalledWith('/comments', { name: 'New Item' });
  });

  it('update puts to /comments/{id}', async () => {
    const { apiClient } = await import('@/lib/api-client');
    ...
    expect(apiClient.put).toHaveBeenCalledWith('/comments/1', { name: 'Updated' });
  });

  it('remove deletes /comments/{id}', async () => {
    const { apiClient } = await import('@/lib/api-client');
