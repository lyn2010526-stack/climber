...
describe('useDocuments', () => {
  ...
  it('create posts to /documents', async () => {
    const { apiClient } = await import('@/lib/api-client');
    ...
    expect(apiClient.post).toHaveBeenCalledWith('/documents', { name: 'New Item' });
  });

  it('update puts to /documents/{id}', async () => {
    const { apiClient } = await import('@/lib/api-client');
    ...
    expect(apiClient.put).toHaveBeenCalledWith('/documents/1', { name: 'Updated' });
  });
