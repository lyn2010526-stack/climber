...
describe('useMessages', () => {
  ...
  it('create posts to /messages', async () => {
    const { apiClient } = await import('@/lib/api-client');
    ...
    expect(apiClient.post).toHaveBeenCalledWith('/messages', { name: 'New Item' });
  });

  it('update puts to /messages/{id}', async () => {
    const { apiClient } = await import('@/lib/api-client');
    ...
    expect(apiClient.put).toHaveBeenCalledWith('/messages/1', { name: 'Updated' });
  });

  it('remove deletes /messages/{id}', async () => {
    const { apiClient } = await import('@/lib/api-client');
