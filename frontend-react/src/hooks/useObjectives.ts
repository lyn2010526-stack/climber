...
type ObjectivesHookReturn = ObjectivesState & ObjectivesActions


export function useObjectives(initialFilters?: Record<string, any>): ObjectivesHookReturn {
    const { showToast } = useToast()
    const { user } = useAuth()
    const queryClient = useQueryClient()
    const abortControllerRef = useRef<AbortController | null>(null)

    const [state, setState] = useState<ObjectivesState>({
        data: null,
        loading: false,
        error: null,
        page: 1,
    ...
    const { data, isLoading, error, refetch } = useQuery({
        queryKey: ['objectives', state.page, state.pageSize, state.filters, state.sortBy, state.sortOrder],
        queryFn: async () => {
