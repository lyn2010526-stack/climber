...
type BlockersHookReturn = BlockersState & BlockersActions


export function useBlockers(initialFilters?: Record<string, any>): BlockersHookReturn {
    const { showToast } = useToast()
    const { user } = useAuth()
    const queryClient = useQueryClient()
    const abortControllerRef = useRef<AbortController | null>(null)

    const [state, setState] = useState<BlockersState>({
        data: null,
        loading: false,
        error: null,
        page: 1,
    ...
    const { data, isLoading, error, refetch } = useQuery({
        queryKey: ['blockers', state.page, state.pageSize, state.filters, state.sortBy, state.sortOrder],
        queryFn: async () => {
    ...
    const createMutation = useMutation({
        mutationFn: async (data: any) => {
            ...
            const response = await apiClient.post('/blockers', data)
            return response.data
        },
        onSuccess: () => {
    ...
    const updateMutation = useMutation({
        mutationFn: async ({ id, data }: { id: number; data: any }) => {
            ...
            const response = await apiClient.put('/blockers/' + id, data)
            return response.data
        },
        onSuccess: () => {
            queryClient.invalidateQueries({ queryKey: ['blockers'] })
    ...
    const deleteMutation = useMutation({
        mutationFn: async (id: number) => {
            ...
            await apiClient.delete('/blockers/' + id)
            return id
        },
        onSuccess: () => {
            queryClient.invalidateQueries({ queryKey: ['blockers'] })
