...
type RisksHookReturn = RisksState & RisksActions


export function useRisks(initialFilters?: Record<string, any>): RisksHookReturn {
    const { showToast } = useToast()
    const { user } = useAuth()
    const queryClient = useQueryClient()
    const abortControllerRef = useRef<AbortController | null>(null)

    const [state, setState] = useState<RisksState>({
        data: null,
        loading: false,
        error: null,
        page: 1,
    ...
    const { data, isLoading, error, refetch } = useQuery({
        queryKey: ['risks', state.page, state.pageSize, state.filters, state.sortBy, state.sortOrder],
        queryFn: async () => {
