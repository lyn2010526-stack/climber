...
type InitiativesHookReturn = InitiativesState & InitiativesActions


export function useInitiatives(initialFilters?: Record<string, any>): InitiativesHookReturn {
    const { showToast } = useToast()
    const { user } = useAuth()
    const queryClient = useQueryClient()
    const abortControllerRef = useRef<AbortController | null>(null)

    const [state, setState] = useState<InitiativesState>({
        data: null,
        loading: false,
        error: null,
        page: 1,
    ...
    const { data, isLoading, error, refetch } = useQuery({
        queryKey: ['initiatives', state.page, state.pageSize, state.filters, state.sortBy, state.sortOrder],
        queryFn: async () => {
    ...
    const updateMutation = useMutation({
        mutationFn: async ({ id, data }: { id: number; data: any }) => {
            ...
            const response = await apiClient.put('/initiatives/' + id, data)
            return response.data
        },
        onSuccess: () => {
            queryClient.invalidateQueries({ queryKey: ['initiatives'] })
