"""Tests for language_detector skill - Detect language of input text."""


import pytest

from app.skills.language_detector_skill import (
    LanguageDetectorConfig,
    LanguageDetectorInputType,
    LanguageDetectorOutputType,
    LanguageDetectorResult,
    LanguageDetectorSkill,
    LanguageDetectorSkillRegistry,
)


@pytest.fixture
def config() -> LanguageDetectorConfig:
...
@pytest.fixture
def skill(config: LanguageDetectorConfig) -> LanguageDetectorSkill:
    return LanguageDetectorSkill(config)


@pytest.fixture
def registry() -> LanguageDetectorSkillRegistry:
    return LanguageDetectorSkillRegistry()
...
class TestLanguageDetectorSkillExecution:
...
class TestLanguageDetectorBatchExecution:
    ...
    async def test_batch_with_configured_size(self):
        config = LanguageDetectorConfig(batch_size=2)
        skill = LanguageDetectorSkill(config)
        items = ["a", "b", "c", "d", "e"]
        results = await skill.execute_batch(items)
...
class TestLanguageDetectorMetrics:
...
class TestLanguageDetectorSkillRegistry:
...
class TestLanguageDetectorMiddleware:
...
class TestLanguageDetectorResult:
    """Tests for result object."""

    def test_success_result(self):
        result = LanguageDetectorResult(success=True, data="output")
        assert result.success is True
        assert result.error is None

    def test_error_result(self):
        result = LanguageDetectorResult(success=False, error="Something went wrong")
        assert result.success is False
        assert result.error == "Something went wrong"


class TestLanguageDetectorEnums:
    """Tests for enum values."""

    def test_input_types(self):
        assert LanguageDetectorInputType.TEXT.value == "text"
        assert LanguageDetectorInputType.JSON.value == "json"
        assert LanguageDetectorInputType.FILE.value == "file"

    def test_output_types(self):
        assert LanguageDetectorOutputType.TEXT.value == "text"
        assert LanguageDetectorOutputType.JSON.value == "json"
        assert LanguageDetectorOutputType.HTML.value == "html"
