"""Tests for ocr_processor skill - Extract text from images."""


import pytest

from app.skills.ocr_processor_skill import (
    OcrProcessorConfig,
    OcrProcessorInputType,
    OcrProcessorOutputType,
    OcrProcessorResult,
    OcrProcessorSkill,
    OcrProcessorSkillRegistry,
)


@pytest.fixture
def config() -> OcrProcessorConfig:
    return OcrProcessorConfig(
        enabled=True,
        timeout_seconds=10,
        max_retries=2,
        validate_input=True,
...
@pytest.fixture
def skill(config: OcrProcessorConfig) -> OcrProcessorSkill:
    return OcrProcessorSkill(config)


@pytest.fixture
def registry() -> OcrProcessorSkillRegistry:
    return OcrProcessorSkillRegistry()


class TestOcrProcessorConfig:
...
class TestOcrProcessorSkillExecution:
...
class TestOcrProcessorBatchExecution:
    ...
    async def test_batch_with_configured_size(self):
        config = OcrProcessorConfig(batch_size=2)
        skill = OcrProcessorSkill(config)
        items = ["a", "b", "c", "d", "e"]
        results = await skill.execute_batch(items)
        assert len(results) == 5


class TestOcrProcessorCache:
...
class TestOcrProcessorSkillRegistry:
...
class TestOcrProcessorMiddleware:
...
class TestOcrProcessorResult:
    """Tests for result object."""

    def test_success_result(self):
        result = OcrProcessorResult(success=True, data="output")
        assert result.success is True
        assert result.error is None

    def test_error_result(self):
        result = OcrProcessorResult(success=False, error="Something went wrong")
        assert result.success is False
        assert result.error == "Something went wrong"


class TestOcrProcessorEnums:
    """Tests for enum values."""

    def test_input_types(self):
        assert OcrProcessorInputType.TEXT.value == "text"
        assert OcrProcessorInputType.JSON.value == "json"
        assert OcrProcessorInputType.FILE.value == "file"

    def test_output_types(self):
        assert OcrProcessorOutputType.TEXT.value == "text"
        assert OcrProcessorOutputType.JSON.value == "json"
        assert OcrProcessorOutputType.HTML.value == "html"
