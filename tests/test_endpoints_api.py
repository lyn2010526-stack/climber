...
class TestEndpointAPI:
    ...
    async def test_get(self, client: AsyncClient):
        response = await client.get('/endpoints/1')
    ...
    async def test_update(self, client: AsyncClient):
        response = await client.put('/endpoints/1', json={'name': 'updated'})
        assert response.status_code in (200, 404)

    @pytest.mark.asyncio
    async def test_delete(self, client: AsyncClient):
        response = await client.delete('/endpoints/1')
