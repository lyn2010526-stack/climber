"""Tests for url_shortener skill - Generate short URLs."""


import pytest

from app.skills.url_shortener_skill import (
    UrlShortenerConfig,
    UrlShortenerInputType,
    UrlShortenerOutputType,
    UrlShortenerResult,
    UrlShortenerSkill,
    UrlShortenerSkillRegistry,
)


@pytest.fixture
def config() -> UrlShortenerConfig:
...
@pytest.fixture
def skill(config: UrlShortenerConfig) -> UrlShortenerSkill:
    return UrlShortenerSkill(config)


@pytest.fixture
def registry() -> UrlShortenerSkillRegistry:
    return UrlShortenerSkillRegistry()


class TestUrlShortenerConfig:
...
class TestUrlShortenerSkillExecution:
...
class TestUrlShortenerBatchExecution:
    ...
    async def test_batch_with_configured_size(self):
        config = UrlShortenerConfig(batch_size=2)
        skill = UrlShortenerSkill(config)
        items = ["a", "b", "c", "d", "e"]
        results = await skill.execute_batch(items)
        assert len(results) == 5


class TestUrlShortenerCache:
...
class TestUrlShortenerSkillRegistry:
...
class TestUrlShortenerMiddleware:
...
class TestUrlShortenerResult:
    """Tests for result object."""

    def test_success_result(self):
        result = UrlShortenerResult(success=True, data="output")
        assert result.success is True
        assert result.error is None

    def test_error_result(self):
        result = UrlShortenerResult(success=False, error="Something went wrong")
        assert result.success is False
        assert result.error == "Something went wrong"


class TestUrlShortenerEnums:
    """Tests for enum values."""

    def test_input_types(self):
        assert UrlShortenerInputType.TEXT.value == "text"
        assert UrlShortenerInputType.JSON.value == "json"
        assert UrlShortenerInputType.FILE.value == "file"

    def test_output_types(self):
        assert UrlShortenerOutputType.TEXT.value == "text"
        assert UrlShortenerOutputType.JSON.value == "json"
        assert UrlShortenerOutputType.HTML.value == "html"
