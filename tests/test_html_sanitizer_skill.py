"""Tests for html_sanitizer skill - Sanitize HTML content."""


import pytest

from app.skills.html_sanitizer_skill import (
    HtmlSanitizerConfig,
    HtmlSanitizerInputType,
    HtmlSanitizerOutputType,
    HtmlSanitizerResult,
    HtmlSanitizerSkill,
    HtmlSanitizerSkillRegistry,
)


@pytest.fixture
def config() -> HtmlSanitizerConfig:
...
@pytest.fixture
def skill(config: HtmlSanitizerConfig) -> HtmlSanitizerSkill:
    return HtmlSanitizerSkill(config)


@pytest.fixture
def registry() -> HtmlSanitizerSkillRegistry:
    return HtmlSanitizerSkillRegistry()


class TestHtmlSanitizerConfig:
...
class TestHtmlSanitizerSkillExecution:
...
class TestHtmlSanitizerBatchExecution:
    ...
    async def test_batch_with_configured_size(self):
        config = HtmlSanitizerConfig(batch_size=2)
        skill = HtmlSanitizerSkill(config)
        items = ["a", "b", "c", "d", "e"]
        results = await skill.execute_batch(items)
        assert len(results) == 5


class TestHtmlSanitizerCache:
...
class TestHtmlSanitizerMetrics:
...
class TestHtmlSanitizerSkillRegistry:
...
class TestHtmlSanitizerMiddleware:
...
class TestHtmlSanitizerResult:
    """Tests for result object."""

    def test_success_result(self):
        result = HtmlSanitizerResult(success=True, data="output")
        assert result.success is True
        assert result.error is None

    def test_error_result(self):
        result = HtmlSanitizerResult(success=False, error="Something went wrong")
        assert result.success is False
        assert result.error == "Something went wrong"


class TestHtmlSanitizerEnums:
    """Tests for enum values."""

    def test_input_types(self):
        assert HtmlSanitizerInputType.TEXT.value == "text"
        assert HtmlSanitizerInputType.JSON.value == "json"
        assert HtmlSanitizerInputType.FILE.value == "file"

    def test_output_types(self):
        assert HtmlSanitizerOutputType.TEXT.value == "text"
        assert HtmlSanitizerOutputType.JSON.value == "json"
        assert HtmlSanitizerOutputType.HTML.value == "html"
