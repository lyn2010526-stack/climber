"""Tests for entity_extractor skill - Extract named entities from text."""


import pytest

from app.skills.entity_extractor_skill import (
    EntityExtractorConfig,
    EntityExtractorInputType,
    EntityExtractorOutputType,
    EntityExtractorResult,
    EntityExtractorSkill,
    EntityExtractorSkillRegistry,
)


@pytest.fixture
def config() -> EntityExtractorConfig:
...
@pytest.fixture
def skill(config: EntityExtractorConfig) -> EntityExtractorSkill:
    return EntityExtractorSkill(config)


@pytest.fixture
def registry() -> EntityExtractorSkillRegistry:
    return EntityExtractorSkillRegistry()
...
class TestEntityExtractorSkillExecution:
...
class TestEntityExtractorBatchExecution:
    ...
    async def test_batch_with_configured_size(self):
        config = EntityExtractorConfig(batch_size=2)
        skill = EntityExtractorSkill(config)
        items = ["a", "b", "c", "d", "e"]
        results = await skill.execute_batch(items)
...
class TestEntityExtractorMetrics:
...
class TestEntityExtractorSkillRegistry:
...
class TestEntityExtractorMiddleware:
...
class TestEntityExtractorResult:
    """Tests for result object."""

    def test_success_result(self):
        result = EntityExtractorResult(success=True, data="output")
        assert result.success is True
        assert result.error is None

    def test_error_result(self):
        result = EntityExtractorResult(success=False, error="Something went wrong")
        assert result.success is False
        assert result.error == "Something went wrong"


class TestEntityExtractorEnums:
    """Tests for enum values."""

    def test_input_types(self):
        assert EntityExtractorInputType.TEXT.value == "text"
        assert EntityExtractorInputType.JSON.value == "json"
        assert EntityExtractorInputType.FILE.value == "file"

    def test_output_types(self):
        assert EntityExtractorOutputType.TEXT.value == "text"
        assert EntityExtractorOutputType.JSON.value == "json"
        assert EntityExtractorOutputType.HTML.value == "html"
