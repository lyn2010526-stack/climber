"""Tests for load data processor."""


from app.processors.load_processor import (
    LoadAggregator,
    LoadProcessor,
    LoadTransformer,
    LoadValidator,
...
class TestLoadTransformer:
    ...
    def test_normalize(self):
        result = LoadTransformer.normalize([10, 20, 30])
        assert result[0] == 0.0
        assert result[-1] == 1.0

    def test_standardize(self):
        result = LoadTransformer.standardize([1, 2, 3, 4, 5])
        assert len(result) == 5

    def test_moving_average(self):
        result = LoadTransformer.moving_average([1, 2, 3, 4, 5], window=3)
        assert result[0] == 2.0
        assert result[-1] == 4.0

    def test_detect_outliers(self):
        data = [1, 2, 3, 4, 5, 100]
        outliers = LoadTransformer.detect_outliers(data)
        assert 5 in outliers

    def test_interpolate_missing(self):
        data = [1.0, None, 3.0]
        result = LoadTransformer.interpolate_missing(data)
        assert result[1] == 2.0
...
class TestLoadAggregator:
    ...
    def test_sum_by_key(self):
        ...
        total = LoadAggregator.sum_by_key(records, 'val')
        assert total == 60

    def test_count_by_key(self):
        records = [{'cat': 'a'}, {'cat': 'b'}, {'cat': 'a'}]
        counts = LoadAggregator.count_by_key(records, 'cat')
        assert counts['a'] == 2

    def test_group_by(self):
        records = [{'type': 'x', 'v': 1}, {'type': 'y', 'v': 2}]
        groups = LoadAggregator.group_by(records, 'type')
        assert 'x' in groups
        assert 'y' in groups

    def test_aggregate_stats(self):
        stats = LoadAggregator.aggregate_stats([1, 2, 3, 4, 5])
...
class TestLoadValidator:
    ...
    def test_validate_schema_valid(self):
        ...
        errors = LoadValidator.validate_schema(record, schema)
        assert len(errors) == 0
    ...
    def test_validate_range(self):
        assert LoadValidator.validate_range(5, 1, 10)
        assert not LoadValidator.validate_range(15, 1, 10)

    def test_validate_required(self):
        record = {'id': 1}
        missing = LoadValidator.validate_required(record, ['id', 'name'])
        assert 'name' in missing

    def test_validate_uniqueness(self):
        records = [{'id': 1}, {'id': 2}, {'id': 1}]
        dups = LoadValidator.validate_uniqueness(records, 'id')
        assert 2 in dups
