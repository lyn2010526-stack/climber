...
class TestMigrationAPI:
    ...
    async def test_get(self, client: AsyncClient):
        response = await client.get('/migrations/1')
    ...
    async def test_update(self, client: AsyncClient):
        response = await client.put('/migrations/1', json={'name': 'updated'})
        assert response.status_code in (200, 404)

    @pytest.mark.asyncio
    async def test_delete(self, client: AsyncClient):
        response = await client.delete('/migrations/1')
