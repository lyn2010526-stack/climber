"""Tests for redis_cloud integration."""


import pytest

from app.integrations.redis_cloud_integration import (
    RedisCloudAuthType,
    RedisCloudClient,
    RedisCloudConfig,
    RedisCloudRateLimiter,
    RedisCloudResponse,
    RedisCloudRetryPolicy,
    RedisCloudWebhookHandler,
)


@pytest.fixture
def config() -> RedisCloudConfig:
...
@pytest.fixture
def client(config: RedisCloudConfig) -> RedisCloudClient:
...
class TestRedisCloudRetryPolicy:
    """Tests for retry policy."""

    def test_default_policy(self):
        policy = RedisCloudRetryPolicy()
        assert policy.max_retries == 3
        assert policy.initial_delay == 1.0

    def test_exponential_delay(self):
        policy = RedisCloudRetryPolicy(initial_delay=1.0, exponential_base=2.0)
        assert policy.get_delay(0) == 1.0
        assert policy.get_delay(1) == 2.0
        assert policy.get_delay(2) == 4.0

    def test_max_delay_cap(self):
        policy = RedisCloudRetryPolicy(initial_delay=1.0, max_delay=5.0)
...
class TestRedisCloudResponse:
    """Tests for response wrapper."""

    def test_success_response(self):
        resp = RedisCloudResponse(status_code=200, headers={}, body={"ok": True}, duration_ms=100)
        assert resp.is_success is True
        assert resp.is_rate_limited is False

    def test_rate_limited_response(self):
        resp = RedisCloudResponse(status_code=429, headers={}, body={}, duration_ms=0)
        assert resp.is_rate_limited is True
        assert resp.is_success is False


class TestRedisCloudWebhookHandler:
    """Tests for webhook handler."""

    @pytest.mark.asyncio
    async def test_handle_known_event(self):
        handler = RedisCloudWebhookHandler("secret")
        called_with = {}
...
class TestRedisCloudRateLimiter:
    """Tests for rate limiter."""

    @pytest.mark.asyncio
    async def test_acquire_within_burst(self):
        limiter = RedisCloudRateLimiter(rate=10, burst=5)
        wait = await limiter.acquire()
        assert wait == 0

    @pytest.mark.asyncio
    async def test_rate_limiting(self):
        limiter = RedisCloudRateLimiter(rate=1, burst=1)
        await limiter.acquire()
        wait = await limiter.acquire()
        assert wait > 0


class TestRedisCloudAuthType:
    """Tests for auth type enum."""

    def test_auth_types(self):
        assert RedisCloudAuthType.API_KEY.value == "api_key"
        assert RedisCloudAuthType.BEARER.value == "bearer"
        assert RedisCloudAuthType.OAUTH2.value == "oauth2"
