...
from app.integrations.bigquery_integration import (
    BigQueryAuthType,
    BigQueryClient,
    BigQueryConfig,
    BigQueryRateLimiter,
...
@pytest.fixture
def client(config: BigQueryConfig) -> BigQueryClient:
...
class TestBigQueryRetryPolicy:
    """Tests for retry policy."""

    def test_default_policy(self):
        policy = BigQueryRetryPolicy()
        assert policy.max_retries == 3
        assert policy.initial_delay == 1.0

    def test_exponential_delay(self):
        policy = BigQueryRetryPolicy(initial_delay=1.0, exponential_base=2.0)
        assert policy.get_delay(0) == 1.0
        assert policy.get_delay(1) == 2.0
        assert policy.get_delay(2) == 4.0

    def test_max_delay_cap(self):
        policy = BigQueryRetryPolicy(initial_delay=1.0, max_delay=5.0)
...
class TestBigQueryResponse:
    """Tests for response wrapper."""

    def test_success_response(self):
        resp = BigQueryResponse(status_code=200, headers={}, body={"ok": True}, duration_ms=100)
        assert resp.is_success is True
        assert resp.is_rate_limited is False

    def test_rate_limited_response(self):
        resp = BigQueryResponse(status_code=429, headers={}, body={}, duration_ms=0)
        assert resp.is_rate_limited is True
        assert resp.is_success is False


class TestBigQueryWebhookHandler:
    """Tests for webhook handler."""

    @pytest.mark.asyncio
    async def test_handle_known_event(self):
        handler = BigQueryWebhookHandler("secret")
        called_with = {}
...
class TestBigQueryRateLimiter:
    """Tests for rate limiter."""

    @pytest.mark.asyncio
    async def test_acquire_within_burst(self):
        limiter = BigQueryRateLimiter(rate=10, burst=5)
        wait = await limiter.acquire()
        assert wait == 0

    @pytest.mark.asyncio
    async def test_rate_limiting(self):
        limiter = BigQueryRateLimiter(rate=1, burst=1)
        await limiter.acquire()
        wait = await limiter.acquire()
        assert wait > 0


class TestBigQueryAuthType:
    """Tests for auth type enum."""

    def test_auth_types(self):
        assert BigQueryAuthType.API_KEY.value == "api_key"
        assert BigQueryAuthType.BEARER.value == "bearer"
        assert BigQueryAuthType.OAUTH2.value == "oauth2"
