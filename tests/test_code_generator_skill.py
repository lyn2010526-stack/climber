"""Tests for code_generator skill - Generate code from descriptions."""


import pytest

from app.skills.code_generator_skill import (
    CodeGeneratorConfig,
    CodeGeneratorInputType,
    CodeGeneratorOutputType,
    CodeGeneratorResult,
    CodeGeneratorSkill,
    CodeGeneratorSkillRegistry,
)
...
@pytest.fixture
def skill(config: CodeGeneratorConfig) -> CodeGeneratorSkill:
    return CodeGeneratorSkill(config)


@pytest.fixture
def registry() -> CodeGeneratorSkillRegistry:
    return CodeGeneratorSkillRegistry()
...
class TestCodeGeneratorSkillExecution:
...
class TestCodeGeneratorBatchExecution:
    ...
    async def test_batch_with_configured_size(self):
        config = CodeGeneratorConfig(batch_size=2)
        skill = CodeGeneratorSkill(config)
        items = ["a", "b", "c", "d", "e"]
        results = await skill.execute_batch(items)
        assert len(results) == 5


class TestCodeGeneratorCache:
...
class TestCodeGeneratorMetrics:
...
class TestCodeGeneratorSkillRegistry:
...
class TestCodeGeneratorMiddleware:
...
class TestCodeGeneratorResult:
    """Tests for result object."""

    def test_success_result(self):
        result = CodeGeneratorResult(success=True, data="output")
        assert result.success is True
        assert result.error is None

    def test_error_result(self):
        result = CodeGeneratorResult(success=False, error="Something went wrong")
        assert result.success is False
        assert result.error == "Something went wrong"


class TestCodeGeneratorEnums:
    """Tests for enum values."""

    def test_input_types(self):
        assert CodeGeneratorInputType.TEXT.value == "text"
        assert CodeGeneratorInputType.JSON.value == "json"
        assert CodeGeneratorInputType.FILE.value == "file"

    def test_output_types(self):
        assert CodeGeneratorOutputType.TEXT.value == "text"
        assert CodeGeneratorOutputType.JSON.value == "json"
        assert CodeGeneratorOutputType.HTML.value == "html"
