...
def test_ws_echo_connected_and_echoes(client):
    with client.websocket_connect("/api/v1/ws/sess-1") as ws:
...
def test_ws_task_snapshot_for_existing_task(client):
    ...
    with client.websocket_connect(f"/api/v1/ws/task/{task['id']}") as ws:
...
def test_ws_task_unknown_task_reports_error(client):
    with client.websocket_connect("/api/v1/ws/task/nonexistent-task") as ws:
        first = ws.receive_json()
...
def test_ws_group_not_found_closes(client):
    with client.websocket_connect("/api/v1/ws/groups/nonexistent-group") as ws:
...
def test_ws_group_ack_and_invalid_json(client):
    gid = _create_group(client)
    with client.websocket_connect(f"/api/v1/ws/groups/{gid}?user_id=alice") as ws:
...
def test_ws_collab_hello_and_ping(client):
    with client.websocket_connect("/api/v1/ws/collab/c1?user_id=bob") as ws:
        ws.send_text(json.dumps({"type": "ping"}))
        assert ws.receive_json() == {"type": "pong"}
        ws.send_text(json.dumps({"type": "hello"}))
        assert ws.receive_json() == {"type": "hello", "session_id": "c1", "user_id": "bob"}


def test_ws_collab_message_broadcast_and_unknown_type(client):
    with client.websocket_connect("/api/v1/ws/collab/c2") as ws:
