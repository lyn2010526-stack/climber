"""Tests for schema data processor."""


from app.processors.schema_processor import (
    SchemaAggregator,
    SchemaProcessor,
    SchemaTransformer,
    SchemaValidator,
...
class TestSchemaTransformer:
    ...
    def test_normalize(self):
        result = SchemaTransformer.normalize([10, 20, 30])
        assert result[0] == 0.0
        assert result[-1] == 1.0

    def test_standardize(self):
        result = SchemaTransformer.standardize([1, 2, 3, 4, 5])
        assert len(result) == 5

    def test_moving_average(self):
        result = SchemaTransformer.moving_average([1, 2, 3, 4, 5], window=3)
        assert result[0] == 2.0
        assert result[-1] == 4.0

    def test_detect_outliers(self):
        data = [1, 2, 3, 4, 5, 100]
        outliers = SchemaTransformer.detect_outliers(data)
        assert 5 in outliers

    def test_interpolate_missing(self):
        data = [1.0, None, 3.0]
        result = SchemaTransformer.interpolate_missing(data)
        assert result[1] == 2.0
...
class TestSchemaAggregator:
    ...
    def test_sum_by_key(self):
        ...
        total = SchemaAggregator.sum_by_key(records, 'val')
        assert total == 60

    def test_count_by_key(self):
        records = [{'cat': 'a'}, {'cat': 'b'}, {'cat': 'a'}]
        counts = SchemaAggregator.count_by_key(records, 'cat')
        assert counts['a'] == 2

    def test_group_by(self):
        records = [{'type': 'x', 'v': 1}, {'type': 'y', 'v': 2}]
        groups = SchemaAggregator.group_by(records, 'type')
        assert 'x' in groups
        assert 'y' in groups

    def test_aggregate_stats(self):
        stats = SchemaAggregator.aggregate_stats([1, 2, 3, 4, 5])
...
class TestSchemaValidator:
    ...
    def test_validate_schema_valid(self):
        ...
        errors = SchemaValidator.validate_schema(record, schema)
        assert len(errors) == 0
    ...
    def test_validate_range(self):
        assert SchemaValidator.validate_range(5, 1, 10)
        assert not SchemaValidator.validate_range(15, 1, 10)

    def test_validate_required(self):
        record = {'id': 1}
        missing = SchemaValidator.validate_required(record, ['id', 'name'])
        assert 'name' in missing

    def test_validate_uniqueness(self):
        records = [{'id': 1}, {'id': 2}, {'id': 1}]
        dups = SchemaValidator.validate_uniqueness(records, 'id')
        assert 2 in dups
