"""Tests for gateway."""

import pytest

from app.modules.gateway_module import (
    GatewayManager,
    GatewayType,
)


class TestGatewayManager:
    """Tests for manager."""

    @pytest.mark.asyncio
    async def test_create_and_get(self):
        manager = GatewayManager()
        await manager.initialize()
        item = await manager.create('test', GatewayType.TYPE_A)
        assert item.name == 'test'
        found = await manager.get(item.id)
        assert found is not None

    @pytest.mark.asyncio
    async def test_update(self):
        manager = GatewayManager()
        item = await manager.create('test', GatewayType.TYPE_A)
        updated = await manager.update(item.id, name='updated')
        assert updated is not None
        assert updated.name == 'updated'

    @pytest.mark.asyncio
    async def test_delete(self):
        manager = GatewayManager()
        item = await manager.create('test', GatewayType.TYPE_A)
        result = await manager.delete(item.id)
        assert result is True

    @pytest.mark.asyncio
    async def test_list_items(self):
        manager = GatewayManager()
        await manager.create('a', GatewayType.TYPE_A)
        await manager.create('b', GatewayType.TYPE_B)
        result = await manager.list_items()
        assert result.total == 2
