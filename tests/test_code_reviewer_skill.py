"""Tests for code_reviewer skill - Review code for issues and improvements."""


import pytest

from app.skills.code_reviewer_skill import (
    CodeReviewerConfig,
    CodeReviewerInputType,
    CodeReviewerOutputType,
    CodeReviewerResult,
    CodeReviewerSkill,
    CodeReviewerSkillRegistry,
)


@pytest.fixture
def config() -> CodeReviewerConfig:
...
@pytest.fixture
def skill(config: CodeReviewerConfig) -> CodeReviewerSkill:
    return CodeReviewerSkill(config)


@pytest.fixture
def registry() -> CodeReviewerSkillRegistry:
    return CodeReviewerSkillRegistry()
...
class TestCodeReviewerSkillExecution:
...
class TestCodeReviewerBatchExecution:
    ...
    async def test_batch_with_configured_size(self):
        config = CodeReviewerConfig(batch_size=2)
        skill = CodeReviewerSkill(config)
        items = ["a", "b", "c", "d", "e"]
        results = await skill.execute_batch(items)
        assert len(results) == 5


class TestCodeReviewerCache:
...
class TestCodeReviewerSkillRegistry:
...
class TestCodeReviewerMiddleware:
...
class TestCodeReviewerResult:
    """Tests for result object."""

    def test_success_result(self):
        result = CodeReviewerResult(success=True, data="output")
        assert result.success is True
        assert result.error is None

    def test_error_result(self):
        result = CodeReviewerResult(success=False, error="Something went wrong")
        assert result.success is False
        assert result.error == "Something went wrong"


class TestCodeReviewerEnums:
    """Tests for enum values."""

    def test_input_types(self):
        assert CodeReviewerInputType.TEXT.value == "text"
        assert CodeReviewerInputType.JSON.value == "json"
        assert CodeReviewerInputType.FILE.value == "file"

    def test_output_types(self):
        assert CodeReviewerOutputType.TEXT.value == "text"
        assert CodeReviewerOutputType.JSON.value == "json"
        assert CodeReviewerOutputType.HTML.value == "html"
