"""Tests for text_summarizer skill - Summarize long text documents."""


import pytest

from app.skills.text_summarizer_skill import (
    TextSummarizerConfig,
    TextSummarizerInputType,
    TextSummarizerOutputType,
    TextSummarizerResult,
    TextSummarizerSkill,
    TextSummarizerSkillRegistry,
)


@pytest.fixture
def config() -> TextSummarizerConfig:
...
@pytest.fixture
def skill(config: TextSummarizerConfig) -> TextSummarizerSkill:
    return TextSummarizerSkill(config)


@pytest.fixture
def registry() -> TextSummarizerSkillRegistry:
    return TextSummarizerSkillRegistry()


class TestTextSummarizerConfig:
...
class TestTextSummarizerSkillExecution:
...
class TestTextSummarizerBatchExecution:
    ...
    async def test_batch_with_configured_size(self):
        config = TextSummarizerConfig(batch_size=2)
        skill = TextSummarizerSkill(config)
        items = ["a", "b", "c", "d", "e"]
        results = await skill.execute_batch(items)
        assert len(results) == 5


class TestTextSummarizerCache:
...
class TestTextSummarizerMetrics:
...
class TestTextSummarizerSkillRegistry:
...
class TestTextSummarizerMiddleware:
...
class TestTextSummarizerResult:
    """Tests for result object."""

    def test_success_result(self):
        result = TextSummarizerResult(success=True, data="output")
        assert result.success is True
        assert result.error is None

    def test_error_result(self):
        result = TextSummarizerResult(success=False, error="Something went wrong")
        assert result.success is False
        assert result.error == "Something went wrong"


class TestTextSummarizerEnums:
    """Tests for enum values."""

    def test_input_types(self):
        assert TextSummarizerInputType.TEXT.value == "text"
        assert TextSummarizerInputType.JSON.value == "json"
        assert TextSummarizerInputType.FILE.value == "file"

    def test_output_types(self):
        assert TextSummarizerOutputType.TEXT.value == "text"
        assert TextSummarizerOutputType.JSON.value == "json"
        assert TextSummarizerOutputType.HTML.value == "html"
