...
class SystemMetricType(StrEnum):
    """Metric type."""
    COUNTER = 'counter'
    GAUGE = 'gauge'
    HISTOGRAM = 'histogram'
...
class SystemMonitor:
    ...
    def create_alert(self, alert: SystemAlert) -> None:
        """Create alert."""
        self._alerts.append(alert)

    def get_alerts(self, severity: str | None = None, acknowledged: bool | None = None) -> list[SystemAlert]:
        """Get alerts."""
        alerts = self._alerts
        if severity:
    ...
    def register_health_check(self, name: str, check_fn: Callable) -> None:
        ...
        self._health_checks[name] = SystemHealthCheck(name=name)

    async def run_health_checks(self) -> dict[str, SystemHealthCheck]:
