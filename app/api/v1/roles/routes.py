"""API module: roles - REST endpoints for roles."""
...
class RoleService:
    ...
    async def create(
        self, data: RoleCreateRequest, user_id: int
    ) -> dict[str, Any]:
        """Create new record."""
        now = datetime.utcnow()
        record = {
    ...
    async def update(
        self, record_id: int, data: RoleUpdateRequest
    ) -> dict[str, Any] | None:
        """Update record."""
...
@router.get('/{record_id}', response_model=RoleResponse)
async def get_roles(
    record_id: int,
    db: AsyncSession = Depends(get_db),
    user = Depends(get_current_user),
...
@router.put('/{record_id}', response_model=RoleResponse)
