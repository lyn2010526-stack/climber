"""API module: files - REST endpoints for files."""
...
class FileCreateRequest(BaseModel):
    """Request model for creating files."""
...
class FileService:
    ...
    async def create(
        self, data: FileCreateRequest, user_id: int
    ) -> dict[str, Any]:
        """Create new record."""
        now = datetime.utcnow()
        record = {
    ...
    async def update(
        self, record_id: int, data: FileUpdateRequest
    ) -> dict[str, Any] | None:
        """Update record."""
...
@router.get('/{record_id}', response_model=FileResponse)
async def get_files(
    record_id: int,
    db: AsyncSession = Depends(get_db),
    user = Depends(get_current_user),
...
@router.put('/{record_id}', response_model=FileResponse)
