"""API module: labels - REST endpoints for labels."""
...
class LabelService:
    """Service layer for labels."""

    def __init__(self, db: AsyncSession):
        self.db = db

    async def create(
        self, data: LabelCreateRequest, user_id: int
    ) -> dict[str, Any]:
        """Create new record."""
        now = datetime.utcnow()
        record = {
    ...
    async def update(
        self, record_id: int, data: LabelUpdateRequest
    ) -> dict[str, Any] | None:
        """Update record."""
...
@router.get('/{record_id}', response_model=LabelResponse)
...
@router.put('/{record_id}', response_model=LabelResponse)
