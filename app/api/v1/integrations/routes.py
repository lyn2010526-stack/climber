"""API module: integrations - REST endpoints for integrations."""
...
class IntegrationService:
    ...
    async def create(
        self, data: IntegrationCreateRequest, user_id: int
    ) -> dict[str, Any]:
        """Create new record."""
        now = datetime.utcnow()
        record = {
    ...
    async def update(
        self, record_id: int, data: IntegrationUpdateRequest
    ) -> dict[str, Any] | None:
        """Update record."""
...
@router.get('/{record_id}', response_model=IntegrationResponse)
...
async def list_integrations(
    ...
    """List integrations records."""
    svc = IntegrationService(db)
    result = await svc.list(page=page, page_size=page_size, status=status, search=search)
    return result


@router.put('/{record_id}', response_model=IntegrationResponse)
