"""API module: tasks - REST endpoints for tasks."""
...
class TaskService:
    ...
    async def create(
        self, data: TaskCreateRequest, user_id: int
    ) -> dict[str, Any]:
        """Create new record."""
        now = datetime.utcnow()
        record = {
    ...
    async def update(
        self, record_id: int, data: TaskUpdateRequest
    ) -> dict[str, Any] | None:
        """Update record."""
...
@router.get('/{record_id}', response_model=TaskResponse)
async def get_tasks(
    record_id: int,
    db: AsyncSession = Depends(get_db),
    user = Depends(get_current_user),
...
@router.put('/{record_id}', response_model=TaskResponse)
