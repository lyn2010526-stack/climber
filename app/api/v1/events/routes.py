"""API module: events - REST endpoints for events."""
...
class EventService:
    ...
    async def create(
        self, data: EventCreateRequest, user_id: int
    ) -> dict[str, Any]:
        """Create new record."""
        now = datetime.utcnow()
        record = {
    ...
    async def update(
        self, record_id: int, data: EventUpdateRequest
    ) -> dict[str, Any] | None:
        """Update record."""
...
@router.get('/{record_id}', response_model=EventResponse)
...
@router.put('/{record_id}', response_model=EventResponse)
