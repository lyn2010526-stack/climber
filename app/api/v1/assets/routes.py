"""API module: assets - REST endpoints for assets."""
...
class AssetService:
    """Service layer for assets."""

    def __init__(self, db: AsyncSession):
        self.db = db

    async def create(
        self, data: AssetCreateRequest, user_id: int
    ) -> dict[str, Any]:
        """Create new record."""
        now = datetime.utcnow()
        record = {
    ...
    async def update(
        self, record_id: int, data: AssetUpdateRequest
    ) -> dict[str, Any] | None:
        """Update record."""
...
@router.put('/{record_id}', response_model=AssetResponse)
