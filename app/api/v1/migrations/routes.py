"""API module: migrations - REST endpoints for migrations."""
...
class MigrationUpdateRequest(BaseModel):
    """Request model for updating migrations."""
...
class MigrationService:
    """Service layer for migrations."""

    def __init__(self, db: AsyncSession):
        self.db = db

    async def create(
        self, data: MigrationCreateRequest, user_id: int
    ) -> dict[str, Any]:
        """Create new record."""
        now = datetime.utcnow()
        record = {
    ...
    async def update(
        self, record_id: int, data: MigrationUpdateRequest
    ) -> dict[str, Any] | None:
        """Update record."""
...
@router.get('/{record_id}', response_model=MigrationResponse)
async def get_migrations(
    record_id: int,
    db: AsyncSession = Depends(get_db),
    user = Depends(get_current_user),
):
    """Get migrations by ID."""
    svc = MigrationService(db)
