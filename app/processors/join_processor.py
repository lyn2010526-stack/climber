"""Data processor: join - Handles data transformation and processing."""
...
@dataclass
class JoinConfig:
    """Configuration for join processor."""
    batch_size: int = 1000
    max_retries: int = 3
...
class JoinProcessor:
    """Main processor for join."""

    def __init__(self, config: JoinConfig | None = None):
        self.config = config or JoinConfig()
        self._cache: dict[str, Any] = {}
        self._metrics = JoinMetrics()
        self._error_log: list[dict[str, Any]] = []

    @property
    def metrics(self) -> JoinMetrics:
        """Get current metrics."""
        ...
        for i in range(0, len(records), self.config.batch_size):
            ...
            for record in batch:
                try:
                    ...
                    failed += 1
                    errors.append(str(e))
                    self._log_error(record, str(e), 12)
