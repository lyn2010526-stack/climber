"""Data processor: merge - Handles data transformation and processing."""
...
@dataclass
class MergeConfig:
    """Configuration for merge processor."""
    batch_size: int = 1000
    max_retries: int = 3
...
class MergeProcessor:
    """Main processor for merge."""

    def __init__(self, config: MergeConfig | None = None):
        self.config = config or MergeConfig()
        self._cache: dict[str, Any] = {}
        self._metrics = MergeMetrics()
        self._error_log: list[dict[str, Any]] = []

    @property
    def metrics(self) -> MergeMetrics:
        """Get current metrics."""
        ...
        for i in range(0, len(records), self.config.batch_size):
            ...
            for record in batch:
                try:
                    ...
                    failed += 1
                    errors.append(str(e))
                    self._log_error(record, str(e), 13)
