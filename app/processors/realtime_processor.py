...
class RealtimeProcessor:
    ...
    def __init__(self, config: RealtimeProcessorConfig | None = None):
        self.config = config or RealtimeProcessorConfig()
    ...
    async def process(self, data: Any) -> RealtimeProcessorResult:
        """Process data."""
        result = RealtimeProcessorResult()
        try:
            processed = data
    ...
    async def process_batch(self, items: list[Any]) -> list[RealtimeProcessorResult]:
        """Process batch."""
        results = []
...
class RealtimeProcessorPipeline:
    """Processor pipeline."""

    def __init__(self):
        self._processors: list[RealtimeProcessor] = []

    def add_processor(self, processor: RealtimeProcessor) -> None:
        """Add processor."""
        self._processors.append(processor)

    async def execute(self, data: Any) -> RealtimeProcessorResult:
        """Execute pipeline."""
        result = RealtimeProcessorResult(success=True, data=data)
