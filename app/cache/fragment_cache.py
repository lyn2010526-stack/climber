...
class FragmentCache:
    ...
    def __init__(self, config: FragmentCacheConfig | None = None):
        self.config = config or FragmentCacheConfig()
        self._store: dict[str, FragmentCacheEntry] = {}
    ...
    def set(self, key: str, value: Any, ttl: int | None = None) -> None:
        ...
        with self._lock:
            ...
            self._store[key] = FragmentCacheEntry(
...
def fragment_cache_key(*args, **kwargs) -> str:
    """Generate cache key."""
    key_data = json.dumps({'args': args, 'kwargs': kwargs}, sort_keys=True, default=str)
    return hashlib.md5(key_data.encode()).hexdigest()


def fragment_cached(cache: FragmentCache, ttl: int | None = None):
    """Cache decorator."""
    def decorator(fn: Callable) -> Callable:
        def wrapper(*args, **kwargs):
            key = fragment_cache_key(fn.__name__, *args, **kwargs)
