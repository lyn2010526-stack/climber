...
class CookieCache:
    ...
    def __init__(self, config: CookieCacheConfig | None = None):
        self.config = config or CookieCacheConfig()
        self._store: dict[str, CookieCacheEntry] = {}
    ...
    def set(self, key: str, value: Any, ttl: int | None = None) -> None:
        ...
        with self._lock:
            ...
            self._store[key] = CookieCacheEntry(
...
def cookie_cache_key(*args, **kwargs) -> str:
    """Generate cache key."""
    key_data = json.dumps({'args': args, 'kwargs': kwargs}, sort_keys=True, default=str)
    return hashlib.md5(key_data.encode()).hexdigest()


def cookie_cached(cache: CookieCache, ttl: int | None = None):
    """Cache decorator."""
    def decorator(fn: Callable) -> Callable:
        def wrapper(*args, **kwargs):
            key = cookie_cache_key(fn.__name__, *args, **kwargs)
