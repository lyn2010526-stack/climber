...
class FileCache:
    ...
    def __init__(self, config: FileCacheConfig | None = None):
        self.config = config or FileCacheConfig()
        self._store: dict[str, FileCacheEntry] = {}
    ...
    def set(self, key: str, value: Any, ttl: int | None = None) -> None:
        ...
        with self._lock:
            ...
            self._store[key] = FileCacheEntry(
...
def file_cache_key(*args, **kwargs) -> str:
    """Generate cache key."""
    key_data = json.dumps({'args': args, 'kwargs': kwargs}, sort_keys=True, default=str)
    return hashlib.md5(key_data.encode()).hexdigest()


def file_cached(cache: FileCache, ttl: int | None = None):
    """Cache decorator."""
    def decorator(fn: Callable) -> Callable:
        def wrapper(*args, **kwargs):
            key = file_cache_key(fn.__name__, *args, **kwargs)
