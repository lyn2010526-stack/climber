...
class ProxyCache:
    ...
    def __init__(self, config: ProxyCacheConfig | None = None):
        self.config = config or ProxyCacheConfig()
        self._store: dict[str, ProxyCacheEntry] = {}
    ...
    def set(self, key: str, value: Any, ttl: int | None = None) -> None:
        ...
        with self._lock:
            ...
            self._store[key] = ProxyCacheEntry(
...
def proxy_cache_key(*args, **kwargs) -> str:
    """Generate cache key."""
    key_data = json.dumps({'args': args, 'kwargs': kwargs}, sort_keys=True, default=str)
    return hashlib.md5(key_data.encode()).hexdigest()


def proxy_cached(cache: ProxyCache, ttl: int | None = None):
    """Cache decorator."""
    def decorator(fn: Callable) -> Callable:
        def wrapper(*args, **kwargs):
            key = proxy_cache_key(fn.__name__, *args, **kwargs)
