...
class L3Cache:
    ...
    def __init__(self, config: L3CacheConfig | None = None):
        self.config = config or L3CacheConfig()
        self._store: dict[str, L3CacheEntry] = {}
...
def l3_cache_key(*args, **kwargs) -> str:
    """Generate cache key."""
    key_data = json.dumps({'args': args, 'kwargs': kwargs}, sort_keys=True, default=str)
    return hashlib.md5(key_data.encode()).hexdigest()


def l3_cached(cache: L3Cache, ttl: int | None = None):
    """Cache decorator."""
    def decorator(fn: Callable) -> Callable:
        def wrapper(*args, **kwargs):
            key = l3_cache_key(fn.__name__, *args, **kwargs)
