...
class L1Cache:
    ...
    def __init__(self, config: L1CacheConfig | None = None):
        self.config = config or L1CacheConfig()
        self._store: dict[str, L1CacheEntry] = {}
...
def l1_cache_key(*args, **kwargs) -> str:
    """Generate cache key."""
    key_data = json.dumps({'args': args, 'kwargs': kwargs}, sort_keys=True, default=str)
    return hashlib.md5(key_data.encode()).hexdigest()


def l1_cached(cache: L1Cache, ttl: int | None = None):
    """Cache decorator."""
    def decorator(fn: Callable) -> Callable:
        def wrapper(*args, **kwargs):
            key = l1_cache_key(fn.__name__, *args, **kwargs)
