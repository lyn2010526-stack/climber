...
class LocalCache:
    ...
    def __init__(self, config: LocalCacheConfig | None = None):
        self.config = config or LocalCacheConfig()
        self._store: dict[str, LocalCacheEntry] = {}
    ...
    def set(self, key: str, value: Any, ttl: int | None = None) -> None:
        ...
        with self._lock:
            ...
            self._store[key] = LocalCacheEntry(
...
def local_cache_key(*args, **kwargs) -> str:
    """Generate cache key."""
    key_data = json.dumps({'args': args, 'kwargs': kwargs}, sort_keys=True, default=str)
    return hashlib.md5(key_data.encode()).hexdigest()


def local_cached(cache: LocalCache, ttl: int | None = None):
    """Cache decorator."""
    def decorator(fn: Callable) -> Callable:
        def wrapper(*args, **kwargs):
            key = local_cache_key(fn.__name__, *args, **kwargs)
