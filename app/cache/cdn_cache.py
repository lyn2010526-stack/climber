...
class CdnCache:
    ...
    def __init__(self, config: CdnCacheConfig | None = None):
        self.config = config or CdnCacheConfig()
        self._store: dict[str, CdnCacheEntry] = {}
...
def cdn_cache_key(*args, **kwargs) -> str:
    """Generate cache key."""
    key_data = json.dumps({'args': args, 'kwargs': kwargs}, sort_keys=True, default=str)
    return hashlib.md5(key_data.encode()).hexdigest()


def cdn_cached(cache: CdnCache, ttl: int | None = None):
    """Cache decorator."""
    def decorator(fn: Callable) -> Callable:
        def wrapper(*args, **kwargs):
            key = cdn_cache_key(fn.__name__, *args, **kwargs)
