...
class GatewayCache:
    ...
    def __init__(self, config: GatewayCacheConfig | None = None):
        self.config = config or GatewayCacheConfig()
        self._store: dict[str, GatewayCacheEntry] = {}
    ...
    def set(self, key: str, value: Any, ttl: int | None = None) -> None:
        ...
        with self._lock:
            ...
            self._store[key] = GatewayCacheEntry(
...
def gateway_cache_key(*args, **kwargs) -> str:
    """Generate cache key."""
    key_data = json.dumps({'args': args, 'kwargs': kwargs}, sort_keys=True, default=str)
    return hashlib.md5(key_data.encode()).hexdigest()


def gateway_cached(cache: GatewayCache, ttl: int | None = None):
    """Cache decorator."""
    def decorator(fn: Callable) -> Callable:
        def wrapper(*args, **kwargs):
            key = gateway_cache_key(fn.__name__, *args, **kwargs)
