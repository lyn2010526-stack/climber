...
class DatabaseCache:
    ...
    def __init__(self, config: DatabaseCacheConfig | None = None):
        self.config = config or DatabaseCacheConfig()
        self._store: dict[str, DatabaseCacheEntry] = {}
    ...
    def set(self, key: str, value: Any, ttl: int | None = None) -> None:
        ...
        with self._lock:
            ...
            self._store[key] = DatabaseCacheEntry(
...
def database_cache_key(*args, **kwargs) -> str:
    """Generate cache key."""
    key_data = json.dumps({'args': args, 'kwargs': kwargs}, sort_keys=True, default=str)
    return hashlib.md5(key_data.encode()).hexdigest()


def database_cached(cache: DatabaseCache, ttl: int | None = None):
    """Cache decorator."""
    def decorator(fn: Callable) -> Callable:
        def wrapper(*args, **kwargs):
            key = database_cache_key(fn.__name__, *args, **kwargs)
