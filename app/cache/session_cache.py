...
class SessionCache:
    ...
    def __init__(self, config: SessionCacheConfig | None = None):
        self.config = config or SessionCacheConfig()
    ...
    def set(self, key: str, value: Any, ttl: int | None = None) -> None:
        ...
        with self._lock:
            ...
            self._store[key] = SessionCacheEntry(
...
def session_cache_key(*args, **kwargs) -> str:
    """Generate cache key."""
    key_data = json.dumps({'args': args, 'kwargs': kwargs}, sort_keys=True, default=str)
    return hashlib.md5(key_data.encode()).hexdigest()


def session_cached(cache: SessionCache, ttl: int | None = None):
    """Cache decorator."""
    def decorator(fn: Callable) -> Callable:
        def wrapper(*args, **kwargs):
            key = session_cache_key(fn.__name__, *args, **kwargs)
