...
class ApiCache:
    ...
    def __init__(self, config: ApiCacheConfig | None = None):
        ...
        self._store: dict[str, ApiCacheEntry] = {}
...
def api_cache_key(*args, **kwargs) -> str:
    """Generate cache key."""
    key_data = json.dumps({'args': args, 'kwargs': kwargs}, sort_keys=True, default=str)
    return hashlib.md5(key_data.encode()).hexdigest()


def api_cached(cache: ApiCache, ttl: int | None = None):
    """Cache decorator."""
    def decorator(fn: Callable) -> Callable:
        def wrapper(*args, **kwargs):
            key = api_cache_key(fn.__name__, *args, **kwargs)
