...
class PageCache:
    ...
    def __init__(self, config: PageCacheConfig | None = None):
        self.config = config or PageCacheConfig()
        self._store: dict[str, PageCacheEntry] = {}
    ...
    def set(self, key: str, value: Any, ttl: int | None = None) -> None:
        ...
        with self._lock:
            ...
            self._store[key] = PageCacheEntry(
...
def page_cache_key(*args, **kwargs) -> str:
    """Generate cache key."""
    key_data = json.dumps({'args': args, 'kwargs': kwargs}, sort_keys=True, default=str)
    return hashlib.md5(key_data.encode()).hexdigest()


def page_cached(cache: PageCache, ttl: int | None = None):
    """Cache decorator."""
    def decorator(fn: Callable) -> Callable:
        def wrapper(*args, **kwargs):
            key = page_cache_key(fn.__name__, *args, **kwargs)
