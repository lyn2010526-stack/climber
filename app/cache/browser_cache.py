...
class BrowserCache:
    ...
    def __init__(self, config: BrowserCacheConfig | None = None):
        self.config = config or BrowserCacheConfig()
        self._store: dict[str, BrowserCacheEntry] = {}
    ...
    def set(self, key: str, value: Any, ttl: int | None = None) -> None:
        ...
        with self._lock:
            ...
            self._store[key] = BrowserCacheEntry(
...
def browser_cache_key(*args, **kwargs) -> str:
    """Generate cache key."""
    key_data = json.dumps({'args': args, 'kwargs': kwargs}, sort_keys=True, default=str)
    return hashlib.md5(key_data.encode()).hexdigest()


def browser_cached(cache: BrowserCache, ttl: int | None = None):
    """Cache decorator."""
    def decorator(fn: Callable) -> Callable:
        def wrapper(*args, **kwargs):
            key = browser_cache_key(fn.__name__, *args, **kwargs)
