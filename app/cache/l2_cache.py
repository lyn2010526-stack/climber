...
class L2Cache:
    ...
    def __init__(self, config: L2CacheConfig | None = None):
        self.config = config or L2CacheConfig()
        self._store: dict[str, L2CacheEntry] = {}
...
def l2_cache_key(*args, **kwargs) -> str:
    """Generate cache key."""
    key_data = json.dumps({'args': args, 'kwargs': kwargs}, sort_keys=True, default=str)
    return hashlib.md5(key_data.encode()).hexdigest()


def l2_cached(cache: L2Cache, ttl: int | None = None):
    """Cache decorator."""
    def decorator(fn: Callable) -> Callable:
        def wrapper(*args, **kwargs):
            key = l2_cache_key(fn.__name__, *args, **kwargs)
