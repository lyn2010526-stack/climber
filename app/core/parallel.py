...
class ParallelToolExecutor:
    ...
    def __init__(self, registry: ToolRegistry, timeout_per_tool: float = 30.0, validator: Optional[Validator] = None):
    ...
    async def execute_all(self, tool_calls: list[dict[str, Any]]) -> list[ToolExecutionResult]:
        ...
        for tc in tool_calls:
            ...
            if isinstance(args, str):
                ...
                args = {}
            tasks.append(self._execute_one(name, args))
    ...
    async def execute_sequential(self, tool_calls: list[dict[str, Any]]) -> list[ToolExecutionResult]:
        ...
        for tc in tool_calls:
            ...
            if isinstance(args, str):
                ...
                args = {}
            results.append(await self._execute_one(name, args))
        return results

    async def _execute_one(self, name: str, arguments: dict[str, Any]) -> ToolExecutionResult:
        ...
        if self._validator is not None:
            try:
                ...
                return ToolExecutionResult(tool_name=name, error=f"validator error: {e}", success=False, duration_ms=0)
            if not allowed:
                duration = (asyncio.get_event_loop().time() - start) * 1000
                return ToolExecutionResult(tool_name=name, error=f"blocked by sandbox: {reason}", success=False, duration_ms=duration)
        try:
