"""ML module: named_entity - Machine learning utilities."""
...
class NamedEntityVector:
    ...
    def __add__(self, other: NamedEntityVector) -> NamedEntityVector:
        return NamedEntityVector([a + b for a, b in zip(self.data, other.data, strict=False)])

    def __sub__(self, other: NamedEntityVector) -> NamedEntityVector:
        return NamedEntityVector([a - b for a, b in zip(self.data, other.data, strict=False)])

    def __mul__(self, scalar: float) -> NamedEntityVector:
        return NamedEntityVector([v * scalar for v in self.data])

    def dot(self, other: NamedEntityVector) -> float:
        """Dot product."""
    ...
    def normalize(self) -> NamedEntityVector:
        """Normalize to unit vector."""
        mag = self.magnitude()
        if mag == 0:
            return NamedEntityVector([0.0] * len(self.data))
        return NamedEntityVector([v / mag for v in self.data])

    def cosine_similarity(self, other: NamedEntityVector) -> float:
        """Cosine similarity."""
        dot = self.dot(other)
        mag_product = self.magnitude() * other.magnitude()
        return dot / mag_product if mag_product > 0 else 0.0

    def euclidean_distance(self, other: NamedEntityVector) -> float:
        """Euclidean distance."""
        return math.sqrt(sum((a - b) ** 2 for a, b in zip(self.data, other.data, strict=False)))


@dataclass
class NamedEntityMatrix:
    ...
    def transpose(self) -> NamedEntityMatrix:
        """Matrix transpose."""
        if not self.rows:
            return NamedEntityMatrix([])
        return NamedEntityMatrix([list(col) for col in zip(*self.rows, strict=False)])

    def multiply(self, other: NamedEntityMatrix) -> NamedEntityMatrix:
        ...
        return NamedEntityMatrix(result)


class NamedEntityScaler:
...
class NamedEntityFeatureSelector:
...
class NamedEntityCrossValidator:
