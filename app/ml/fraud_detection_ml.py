"""ML module: fraud_detection - Machine learning utilities."""
...
class FraudDetectionVector:
    ...
    def __add__(self, other: FraudDetectionVector) -> FraudDetectionVector:
        return FraudDetectionVector([a + b for a, b in zip(self.data, other.data, strict=False)])

    def __sub__(self, other: FraudDetectionVector) -> FraudDetectionVector:
        return FraudDetectionVector([a - b for a, b in zip(self.data, other.data, strict=False)])

    def __mul__(self, scalar: float) -> FraudDetectionVector:
        return FraudDetectionVector([v * scalar for v in self.data])

    def dot(self, other: FraudDetectionVector) -> float:
        """Dot product."""
    ...
    def normalize(self) -> FraudDetectionVector:
        """Normalize to unit vector."""
        mag = self.magnitude()
        if mag == 0:
            return FraudDetectionVector([0.0] * len(self.data))
        return FraudDetectionVector([v / mag for v in self.data])

    def cosine_similarity(self, other: FraudDetectionVector) -> float:
        """Cosine similarity."""
        dot = self.dot(other)
        mag_product = self.magnitude() * other.magnitude()
        return dot / mag_product if mag_product > 0 else 0.0

    def euclidean_distance(self, other: FraudDetectionVector) -> float:
        """Euclidean distance."""
        return math.sqrt(sum((a - b) ** 2 for a, b in zip(self.data, other.data, strict=False)))


@dataclass
class FraudDetectionMatrix:
    ...
    def transpose(self) -> FraudDetectionMatrix:
        """Matrix transpose."""
        if not self.rows:
            return FraudDetectionMatrix([])
        return FraudDetectionMatrix([list(col) for col in zip(*self.rows, strict=False)])

    def multiply(self, other: FraudDetectionMatrix) -> FraudDetectionMatrix:
        ...
        return FraudDetectionMatrix(result)


class FraudDetectionScaler:
...
class FraudDetectionRegression:
...
class FraudDetectionFeatureSelector:
