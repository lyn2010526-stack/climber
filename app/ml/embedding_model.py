"""ML Model: embedding - Machine learning model."""
...
class EmbeddingFeatureType(StrEnum):
    """Feature type enum."""
    NUMERIC = 'numeric'
...
class EmbeddingMLModel:
    """ML Model manager."""

    def __init__(self):
        self._models: dict[str, dict[str, Any]] = {}
        self._artifacts: dict[str, EmbeddingModelArtifact] = {}
    ...
    def create_model(
        ...
        hyperparameters: list[EmbeddingHyperParameter] | None = None,
    ) -> str:
    ...
    def train_model(self, model_id: str, training_config: EmbeddingTrainingConfig | None = None) -> dict[str, Any]:
        """Train model."""
        model = self._models.get(model_id)
        if not model:
            return {'success': False, 'error': 'Model not found'}

        config = training_config or EmbeddingTrainingConfig()
        job_id = str(uuid4())
