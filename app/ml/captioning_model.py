"""ML Model: captioning - Machine learning model."""
...
class CaptioningModelType(StrEnum):
    """Model type enum."""
...
class CaptioningModelStatus(StrEnum):
    """Model status enum."""
    DRAFT = 'draft'
    TRAINING = 'training'
    TRAINED = 'trained'
...
class CaptioningFeatureType(StrEnum):
    """Feature type enum."""
    NUMERIC = 'numeric'
...
@dataclass
class CaptioningHyperParameter:
...
class CaptioningMLModel:
    """ML Model manager."""

    def __init__(self):
        self._models: dict[str, dict[str, Any]] = {}
        self._artifacts: dict[str, CaptioningModelArtifact] = {}
    ...
    def create_model(
        ...
        name: str,
        model_type: str,
        features: list[CaptioningFeature] | None = None,
        hyperparameters: list[CaptioningHyperParameter] | None = None,
    ) -> str:
        ...
        self._models[model_id] = {
            ...
            'status': 'draft',
            'metrics': CaptioningModelMetrics(),
    ...
    def train_model(self, model_id: str, training_config: CaptioningTrainingConfig | None = None) -> dict[str, Any]:
        """Train model."""
        model = self._models.get(model_id)
        if not model:
            return {'success': False, 'error': 'Model not found'}

        config = training_config or CaptioningTrainingConfig()
        job_id = str(uuid4())
    ...
    def save_artifact(self, model_id: str, model_data: bytes, format: str = 'pickle') -> str:
        """Save model artifact."""
        artifact = CaptioningModelArtifact(
            model_id=model_id,
            format=format,
