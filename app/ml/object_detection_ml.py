"""ML module: object_detection - Machine learning utilities."""
...
class ObjectDetectionVector:
    ...
    def __add__(self, other: ObjectDetectionVector) -> ObjectDetectionVector:
        return ObjectDetectionVector([a + b for a, b in zip(self.data, other.data, strict=False)])

    def __sub__(self, other: ObjectDetectionVector) -> ObjectDetectionVector:
        return ObjectDetectionVector([a - b for a, b in zip(self.data, other.data, strict=False)])

    def __mul__(self, scalar: float) -> ObjectDetectionVector:
        return ObjectDetectionVector([v * scalar for v in self.data])

    def dot(self, other: ObjectDetectionVector) -> float:
        """Dot product."""
    ...
    def normalize(self) -> ObjectDetectionVector:
        """Normalize to unit vector."""
        mag = self.magnitude()
        if mag == 0:
            return ObjectDetectionVector([0.0] * len(self.data))
        return ObjectDetectionVector([v / mag for v in self.data])

    def cosine_similarity(self, other: ObjectDetectionVector) -> float:
        """Cosine similarity."""
        dot = self.dot(other)
        mag_product = self.magnitude() * other.magnitude()
        return dot / mag_product if mag_product > 0 else 0.0

    def euclidean_distance(self, other: ObjectDetectionVector) -> float:
        """Euclidean distance."""
        return math.sqrt(sum((a - b) ** 2 for a, b in zip(self.data, other.data, strict=False)))


@dataclass
class ObjectDetectionMatrix:
    ...
    def transpose(self) -> ObjectDetectionMatrix:
        """Matrix transpose."""
        if not self.rows:
            return ObjectDetectionMatrix([])
        return ObjectDetectionMatrix([list(col) for col in zip(*self.rows, strict=False)])

    def multiply(self, other: ObjectDetectionMatrix) -> ObjectDetectionMatrix:
        ...
        return ObjectDetectionMatrix(result)
...
class ObjectDetectionRegression:
...
class ObjectDetectionFeatureSelector:
...
class ObjectDetectionCrossValidator:
