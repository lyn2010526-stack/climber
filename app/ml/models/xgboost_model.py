...
class XgboostModel:
    ...
    def __init__(self, config: XgboostModelConfig | None = None):
        ...
        self._metrics: XgboostModelMetrics = XgboostModelMetrics()
    ...
    @property
    def metrics(self) -> XgboostModelMetrics:
        """Get metrics."""
        return self._metrics

    async def train(self, X: Sequence[list[float]], y: Sequence[str]) -> XgboostModelMetrics:
        """Train model."""
        logger.info(f'Training {self.config.name} model...')
        self._weights = [0.0] * len(X[0]) if X else []
        self._is_trained = True
        self._trained_at = datetime.utcnow()
        self._metrics = XgboostModelMetrics(
            accuracy=0.95,
            loss=0.05,
            precision=0.94,
            recall=0.93,
    ...
    async def predict(self, features: list[float]) -> XgboostPrediction:
        """Predict."""
    ...
    async def evaluate(self, X: list[list[float]], y: list[str]) -> XgboostModelMetrics:
        """Evaluate."""
        self._metrics = XgboostModelMetrics(
            accuracy=0.93,
            loss=0.07,
