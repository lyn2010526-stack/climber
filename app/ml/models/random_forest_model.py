...
@dataclass
class RandomForestModelMetrics:
    """Model metrics."""
    accuracy: float = 0.0
    loss: float = 0.0
    precision: float = 0.0
...
class RandomForestModel:
    ...
    def __init__(self, config: RandomForestModelConfig | None = None):
        ...
        self._metrics: RandomForestModelMetrics = RandomForestModelMetrics()
    ...
    @property
    def metrics(self) -> RandomForestModelMetrics:
        """Get metrics."""
        return self._metrics

    async def train(self, X: Sequence[list[float]], y: Sequence[str]) -> RandomForestModelMetrics:
        """Train model."""
        logger.info(f'Training {self.config.name} model...')
        self._weights = [0.0] * len(X[0]) if X else []
        self._is_trained = True
        self._trained_at = datetime.utcnow()
        self._metrics = RandomForestModelMetrics(
            accuracy=0.95,
            loss=0.05,
            precision=0.94,
            recall=0.93,
    ...
    async def predict(self, features: list[float]) -> RandomForestPrediction:
        """Predict."""
    ...
    async def evaluate(self, X: list[list[float]], y: list[str]) -> RandomForestModelMetrics:
        """Evaluate."""
        self._metrics = RandomForestModelMetrics(
            accuracy=0.93,
            loss=0.07,
