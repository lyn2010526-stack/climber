"""ML module: speech_to_text - Machine learning utilities."""
...
class SpeechToTextVector:
    ...
    def __add__(self, other: SpeechToTextVector) -> SpeechToTextVector:
        return SpeechToTextVector([a + b for a, b in zip(self.data, other.data, strict=False)])

    def __sub__(self, other: SpeechToTextVector) -> SpeechToTextVector:
        return SpeechToTextVector([a - b for a, b in zip(self.data, other.data, strict=False)])

    def __mul__(self, scalar: float) -> SpeechToTextVector:
        return SpeechToTextVector([v * scalar for v in self.data])

    def dot(self, other: SpeechToTextVector) -> float:
        """Dot product."""
    ...
    def normalize(self) -> SpeechToTextVector:
        """Normalize to unit vector."""
        mag = self.magnitude()
        if mag == 0:
            return SpeechToTextVector([0.0] * len(self.data))
        return SpeechToTextVector([v / mag for v in self.data])

    def cosine_similarity(self, other: SpeechToTextVector) -> float:
        """Cosine similarity."""
        dot = self.dot(other)
        mag_product = self.magnitude() * other.magnitude()
        return dot / mag_product if mag_product > 0 else 0.0

    def euclidean_distance(self, other: SpeechToTextVector) -> float:
        """Euclidean distance."""
        return math.sqrt(sum((a - b) ** 2 for a, b in zip(self.data, other.data, strict=False)))


@dataclass
class SpeechToTextMatrix:
    ...
    def transpose(self) -> SpeechToTextMatrix:
        """Matrix transpose."""
        if not self.rows:
            return SpeechToTextMatrix([])
        return SpeechToTextMatrix([list(col) for col in zip(*self.rows, strict=False)])

    def multiply(self, other: SpeechToTextMatrix) -> SpeechToTextMatrix:
        ...
        return SpeechToTextMatrix(result)


class SpeechToTextScaler:
...
class SpeechToTextFeatureSelector:
...
class SpeechToTextCrossValidator:
