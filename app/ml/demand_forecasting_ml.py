"""ML module: demand_forecasting - Machine learning utilities."""
...
class DemandForecastingVector:
    ...
    def __add__(self, other: DemandForecastingVector) -> DemandForecastingVector:
        return DemandForecastingVector([a + b for a, b in zip(self.data, other.data, strict=False)])

    def __sub__(self, other: DemandForecastingVector) -> DemandForecastingVector:
        return DemandForecastingVector([a - b for a, b in zip(self.data, other.data, strict=False)])

    def __mul__(self, scalar: float) -> DemandForecastingVector:
        return DemandForecastingVector([v * scalar for v in self.data])

    def dot(self, other: DemandForecastingVector) -> float:
        """Dot product."""
    ...
    def normalize(self) -> DemandForecastingVector:
        """Normalize to unit vector."""
        mag = self.magnitude()
        if mag == 0:
            return DemandForecastingVector([0.0] * len(self.data))
        return DemandForecastingVector([v / mag for v in self.data])

    def cosine_similarity(self, other: DemandForecastingVector) -> float:
        """Cosine similarity."""
        dot = self.dot(other)
        mag_product = self.magnitude() * other.magnitude()
        return dot / mag_product if mag_product > 0 else 0.0

    def euclidean_distance(self, other: DemandForecastingVector) -> float:
        """Euclidean distance."""
        return math.sqrt(sum((a - b) ** 2 for a, b in zip(self.data, other.data, strict=False)))


@dataclass
class DemandForecastingMatrix:
    ...
    def transpose(self) -> DemandForecastingMatrix:
        """Matrix transpose."""
        if not self.rows:
            return DemandForecastingMatrix([])
        return DemandForecastingMatrix([list(col) for col in zip(*self.rows, strict=False)])

    def multiply(self, other: DemandForecastingMatrix) -> DemandForecastingMatrix:
        ...
        return DemandForecastingMatrix(result)


class DemandForecastingScaler:
...
class DemandForecastingEncoder:
...
class DemandForecastingClassifier:
    """Simple classifiers."""
...
class DemandForecastingCluster:
...
class DemandForecastingRegression:
...
class DemandForecastingFeatureSelector:
...
class DemandForecastingCrossValidator:
