"""ML Model: optical_flow - Machine learning model."""
...
class OpticalFlowModelType(StrEnum):
    """Model type enum."""
...
class OpticalFlowModelStatus(StrEnum):
    """Model status enum."""
    DRAFT = 'draft'
    TRAINING = 'training'
    TRAINED = 'trained'
...
class OpticalFlowFeatureType(StrEnum):
    """Feature type enum."""
    NUMERIC = 'numeric'
...
@dataclass
class OpticalFlowHyperParameter:
...
@dataclass
class OpticalFlowTrainingConfig:
...
@dataclass
class OpticalFlowModelArtifact:
...
class OpticalFlowMLModel:
    """ML Model manager."""

    def __init__(self):
        self._models: dict[str, dict[str, Any]] = {}
        self._artifacts: dict[str, OpticalFlowModelArtifact] = {}
    ...
    def create_model(
        ...
        name: str,
        model_type: str,
        features: list[OpticalFlowFeature] | None = None,
        hyperparameters: list[OpticalFlowHyperParameter] | None = None,
    ) -> str:
        ...
        self._models[model_id] = {
            ...
            'status': 'draft',
            'metrics': OpticalFlowModelMetrics(),
    ...
    def train_model(self, model_id: str, training_config: OpticalFlowTrainingConfig | None = None) -> dict[str, Any]:
        """Train model."""
        model = self._models.get(model_id)
        if not model:
            return {'success': False, 'error': 'Model not found'}

        config = training_config or OpticalFlowTrainingConfig()
        job_id = str(uuid4())
    ...
    def save_artifact(self, model_id: str, model_data: bytes, format: str = 'pickle') -> str:
        """Save model artifact."""
        artifact = OpticalFlowModelArtifact(
            model_id=model_id,
            format=format,
