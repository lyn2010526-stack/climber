"""ML module: recommendation - Machine learning utilities."""
...
class RecommendationVector:
    ...
    def __add__(self, other: RecommendationVector) -> RecommendationVector:
        return RecommendationVector([a + b for a, b in zip(self.data, other.data, strict=False)])

    def __sub__(self, other: RecommendationVector) -> RecommendationVector:
        return RecommendationVector([a - b for a, b in zip(self.data, other.data, strict=False)])

    def __mul__(self, scalar: float) -> RecommendationVector:
        return RecommendationVector([v * scalar for v in self.data])

    def dot(self, other: RecommendationVector) -> float:
        """Dot product."""
    ...
    def normalize(self) -> RecommendationVector:
        """Normalize to unit vector."""
        mag = self.magnitude()
        if mag == 0:
            return RecommendationVector([0.0] * len(self.data))
        return RecommendationVector([v / mag for v in self.data])

    def cosine_similarity(self, other: RecommendationVector) -> float:
        """Cosine similarity."""
        dot = self.dot(other)
        mag_product = self.magnitude() * other.magnitude()
        return dot / mag_product if mag_product > 0 else 0.0

    def euclidean_distance(self, other: RecommendationVector) -> float:
        """Euclidean distance."""
...
class RecommendationMatrix:
    ...
    def transpose(self) -> RecommendationMatrix:
        """Matrix transpose."""
        if not self.rows:
            return RecommendationMatrix([])
        return RecommendationMatrix([list(col) for col in zip(*self.rows, strict=False)])

    def multiply(self, other: RecommendationMatrix) -> RecommendationMatrix:
        ...
        return RecommendationMatrix(result)


class RecommendationScaler:
...
class RecommendationRegression:
...
class RecommendationFeatureSelector:
...
class RecommendationCrossValidator:
