"""Report: forecast - Report generation."""
...
class ForecastReportType(StrEnum):
    """Report type."""
    SUMMARY = 'summary'
    DETAILED = 'detailed'
...
class ForecastReportFormat(StrEnum):
    """Report format."""
    JSON = 'json'
    CSV = 'csv'
    PDF = 'pdf'
...
class ForecastReportConfig:
    ...
    format: str = 'json'
    columns: list[ForecastReportColumn] = field(default_factory=list)
...
class ForecastReportGenerator:
    ...
    def __init__(self):
        self._reports: dict[str, ForecastReportResult] = {}
        self._templates: dict[str, ForecastReportConfig] = {}

    def generate(self, config: ForecastReportConfig) -> ForecastReportResult:
        """Generate report."""
        data = self._fetch_data(config)
        summary = self._compute_summary(data)

        result = ForecastReportResult(
    ...
    def _fetch_data(self, config: ForecastReportConfig) -> list[dict[str, Any]]:
        """Fetch report data."""
    ...
    def save_template(self, name: str, config: ForecastReportConfig) -> None:
        """Save template."""
        self._templates[name] = config

    def get_template(self, name: str) -> ForecastReportConfig | None:
        """Get template."""
