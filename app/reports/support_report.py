...
class SupportReportType(StrEnum):
    """Report type."""
    SUMMARY = 'summary'
    DETAILED = 'detailed'
...
class SupportReportFormat(StrEnum):
    """Report format."""
    JSON = 'json'
    CSV = 'csv'
    PDF = 'pdf'
...
@dataclass
class SupportReportConfig:
    """Report configuration."""
    title: str = ''
    description: str = ''
    report_type: str = 'summary'
    format: str = 'json'
    columns: list[SupportReportColumn] = field(default_factory=list)
...
class SupportReportGenerator:
    ...
    def __init__(self):
        self._reports: dict[str, SupportReportResult] = {}
        self._templates: dict[str, SupportReportConfig] = {}

    def generate(self, config: SupportReportConfig) -> SupportReportResult:
        """Generate report."""
        data = self._fetch_data(config)
        summary = self._compute_summary(data)

        result = SupportReportResult(
    ...
    def _fetch_data(self, config: SupportReportConfig) -> list[dict[str, Any]]:
        """Fetch report data."""
    ...
    def save_template(self, name: str, config: SupportReportConfig) -> None:
        """Save template."""
        self._templates[name] = config

    def get_template(self, name: str) -> SupportReportConfig | None:
        """Get template."""
