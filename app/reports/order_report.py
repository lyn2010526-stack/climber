"""Report: order - Report generation."""
...
class OrderReportFormat(StrEnum):
    """Report format."""
    JSON = 'json'
    CSV = 'csv'
    PDF = 'pdf'
...
class OrderReportConfig:
    ...
    format: str = 'json'
    columns: list[OrderReportColumn] = field(default_factory=list)
...
class OrderReportGenerator:
    ...
    def __init__(self):
        self._reports: dict[str, OrderReportResult] = {}
        self._templates: dict[str, OrderReportConfig] = {}

    def generate(self, config: OrderReportConfig) -> OrderReportResult:
        """Generate report."""
        data = self._fetch_data(config)
        summary = self._compute_summary(data)

        result = OrderReportResult(
    ...
    def _fetch_data(self, config: OrderReportConfig) -> list[dict[str, Any]]:
        """Fetch report data."""
    ...
    def save_template(self, name: str, config: OrderReportConfig) -> None:
        """Save template."""
        self._templates[name] = config

    def get_template(self, name: str) -> OrderReportConfig | None:
        """Get template."""
