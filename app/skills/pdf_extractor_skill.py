"""Skill module: pdf_extractor - Extract text from PDF files."""
...
class PdfExtractorInputType(StrEnum):
    TEXT = "text"
    JSON = "json"
    FILE = "file"
    URL = "url"
...
class PdfExtractorOutputType(StrEnum):
    TEXT = "text"
    JSON = "json"
    HTML = "html"
    MARKDOWN = "markdown"
...
class PdfExtractorConfig:
    """Configuration for pdf_extractor skill."""
    enabled: bool = True
...
class PdfExtractorResult:
    """Result of pdf_extractor skill execution."""
    success: bool
    data: Any = None
    error: str | None = None
    duration_ms: float = 0
...
class PdfExtractorMetrics:
    """Metrics tracking for pdf_extractor skill."""
    total_calls: int = 0
...
class PdfExtractorSkill:
    ...
    def __init__(self, config: PdfExtractorConfig | None = None):
        self.config = config or PdfExtractorConfig()
        self.metrics = PdfExtractorMetrics()
    ...
    async def execute(self, input_data: Any, input_type: str = "text", **kwargs: Any) -> PdfExtractorResult:
        """Execute the skill."""
        import time
        start = time.time()
        ...
        try:
            ...
            return PdfExtractorResult(success=True, data=result_data, duration_ms=duration)
        except Exception as e:
            duration = (time.time() - start) * 1000
            self.metrics.record_call(False, duration, type(e).__name__)
            logger.error("PdfExtractor skill failed", error=str(e))
            return PdfExtractorResult(success=False, error=str(e), duration_ms=duration)

    async def execute_batch(self, items: list[Any], **kwargs: Any) -> list[PdfExtractorResult]:
...
class PdfExtractorSkillRegistry:
    """Registry for managing pdf_extractor skill instances."""

    def __init__(self):
        self._skills: dict[str, PdfExtractorSkill] = {}
        self._configs: dict[str, PdfExtractorConfig] = {}

    def register(self, name: str, skill: PdfExtractorSkill) -> None:
        """Register a skill instance."""
        self._skills[name] = skill
        logger.info("Registered PdfExtractor skill", name=name)

    def get(self, name: str) -> PdfExtractorSkill | None:
        """Get skill by name."""
    ...
    async def execute(self, name: str, data: Any, **kwargs: Any) -> PdfExtractorResult:
        """Execute a named skill."""
        skill = self.get(name)
        if not skill:
            return PdfExtractorResult(success=False, error=f"Skill not found: {name}")
