"""Skill module: html_sanitizer - Sanitize HTML content."""
...
class HtmlSanitizerInputType(StrEnum):
    TEXT = "text"
    JSON = "json"
    FILE = "file"
    URL = "url"
...
class HtmlSanitizerOutputType(StrEnum):
    TEXT = "text"
    JSON = "json"
    HTML = "html"
    MARKDOWN = "markdown"
...
class HtmlSanitizerConfig:
    """Configuration for html_sanitizer skill."""
    enabled: bool = True
...
@dataclass
class HtmlSanitizerResult:
    """Result of html_sanitizer skill execution."""
    success: bool
    data: Any = None
    error: str | None = None
    duration_ms: float = 0
...
@dataclass
class HtmlSanitizerMetrics:
    """Metrics tracking for html_sanitizer skill."""
    total_calls: int = 0
...
class HtmlSanitizerSkill:
    """Sanitize HTML content."""

    def __init__(self, config: HtmlSanitizerConfig | None = None):
        self.config = config or HtmlSanitizerConfig()
        self.metrics = HtmlSanitizerMetrics()
    ...
    async def execute(self, input_data: Any, input_type: str = "text", **kwargs: Any) -> HtmlSanitizerResult:
        """Execute the skill."""
        import time
        start = time.time()
        ...
        try:
            ...
            return HtmlSanitizerResult(success=True, data=result_data, duration_ms=duration)
        except Exception as e:
            duration = (time.time() - start) * 1000
            self.metrics.record_call(False, duration, type(e).__name__)
            logger.error("HtmlSanitizer skill failed", error=str(e))
            return HtmlSanitizerResult(success=False, error=str(e), duration_ms=duration)

    async def execute_batch(self, items: list[Any], **kwargs: Any) -> list[HtmlSanitizerResult]:
...
class HtmlSanitizerSkillRegistry:
    """Registry for managing html_sanitizer skill instances."""

    def __init__(self):
        self._skills: dict[str, HtmlSanitizerSkill] = {}
        self._configs: dict[str, HtmlSanitizerConfig] = {}

    def register(self, name: str, skill: HtmlSanitizerSkill) -> None:
        """Register a skill instance."""
        self._skills[name] = skill
        logger.info("Registered HtmlSanitizer skill", name=name)

    def get(self, name: str) -> HtmlSanitizerSkill | None:
        """Get skill by name."""
    ...
    async def execute(self, name: str, data: Any, **kwargs: Any) -> HtmlSanitizerResult:
        """Execute a named skill."""
        skill = self.get(name)
        if not skill:
            return HtmlSanitizerResult(success=False, error=f"Skill not found: {name}")
