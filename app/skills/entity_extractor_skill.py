"""Skill module: entity_extractor - Extract named entities from text."""
...
class EntityExtractorInputType(StrEnum):
    TEXT = "text"
    JSON = "json"
    FILE = "file"
    URL = "url"
...
class EntityExtractorOutputType(StrEnum):
    TEXT = "text"
    JSON = "json"
    HTML = "html"
    MARKDOWN = "markdown"
...
class EntityExtractorConfig:
    """Configuration for entity_extractor skill."""
    enabled: bool = True
...
class EntityExtractorResult:
    """Result of entity_extractor skill execution."""
    success: bool
    data: Any = None
    error: str | None = None
    duration_ms: float = 0
...
@dataclass
class EntityExtractorMetrics:
    """Metrics tracking for entity_extractor skill."""
    total_calls: int = 0
...
class EntityExtractorSkill:
    ...
    def __init__(self, config: EntityExtractorConfig | None = None):
        ...
        self.metrics = EntityExtractorMetrics()
    ...
    async def execute(self, input_data: Any, input_type: str = "text", **kwargs: Any) -> EntityExtractorResult:
        """Execute the skill."""
        import time
        start = time.time()
        ...
        try:
            ...
            return EntityExtractorResult(success=True, data=result_data, duration_ms=duration)
        except Exception as e:
            duration = (time.time() - start) * 1000
            self.metrics.record_call(False, duration, type(e).__name__)
            logger.error("EntityExtractor skill failed", error=str(e))
            return EntityExtractorResult(success=False, error=str(e), duration_ms=duration)

    async def execute_batch(self, items: list[Any], **kwargs: Any) -> list[EntityExtractorResult]:
...
class EntityExtractorSkillRegistry:
    """Registry for managing entity_extractor skill instances."""

    def __init__(self):
        self._skills: dict[str, EntityExtractorSkill] = {}
        self._configs: dict[str, EntityExtractorConfig] = {}

    def register(self, name: str, skill: EntityExtractorSkill) -> None:
        """Register a skill instance."""
        self._skills[name] = skill
        logger.info("Registered EntityExtractor skill", name=name)

    def get(self, name: str) -> EntityExtractorSkill | None:
        """Get skill by name."""
    ...
    async def execute(self, name: str, data: Any, **kwargs: Any) -> EntityExtractorResult:
        """Execute a named skill."""
        skill = self.get(name)
        if not skill:
            return EntityExtractorResult(success=False, error=f"Skill not found: {name}")
