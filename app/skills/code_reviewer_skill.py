"""Skill module: code_reviewer - Review code for issues and improvements."""
...
class CodeReviewerInputType(StrEnum):
    TEXT = "text"
    JSON = "json"
    FILE = "file"
    URL = "url"
...
class CodeReviewerOutputType(StrEnum):
    TEXT = "text"
    JSON = "json"
    HTML = "html"
    MARKDOWN = "markdown"
...
class CodeReviewerConfig:
    """Configuration for code_reviewer skill."""
    enabled: bool = True
...
class CodeReviewerResult:
    """Result of code_reviewer skill execution."""
    success: bool
    data: Any = None
    error: str | None = None
    duration_ms: float = 0
...
@dataclass
class CodeReviewerMetrics:
    """Metrics tracking for code_reviewer skill."""
    total_calls: int = 0
...
class CodeReviewerSkill:
    ...
    def __init__(self, config: CodeReviewerConfig | None = None):
        self.config = config or CodeReviewerConfig()
        self.metrics = CodeReviewerMetrics()
    ...
    async def execute(self, input_data: Any, input_type: str = "text", **kwargs: Any) -> CodeReviewerResult:
        """Execute the skill."""
        import time
        start = time.time()
        ...
        try:
            ...
            return CodeReviewerResult(success=True, data=result_data, duration_ms=duration)
        except Exception as e:
            duration = (time.time() - start) * 1000
            self.metrics.record_call(False, duration, type(e).__name__)
            logger.error("CodeReviewer skill failed", error=str(e))
            return CodeReviewerResult(success=False, error=str(e), duration_ms=duration)

    async def execute_batch(self, items: list[Any], **kwargs: Any) -> list[CodeReviewerResult]:
...
class CodeReviewerSkillRegistry:
    """Registry for managing code_reviewer skill instances."""

    def __init__(self):
        self._skills: dict[str, CodeReviewerSkill] = {}
        self._configs: dict[str, CodeReviewerConfig] = {}

    def register(self, name: str, skill: CodeReviewerSkill) -> None:
        """Register a skill instance."""
        self._skills[name] = skill
        logger.info("Registered CodeReviewer skill", name=name)

    def get(self, name: str) -> CodeReviewerSkill | None:
        """Get skill by name."""
    ...
    async def execute(self, name: str, data: Any, **kwargs: Any) -> CodeReviewerResult:
        """Execute a named skill."""
        skill = self.get(name)
        if not skill:
            return CodeReviewerResult(success=False, error=f"Skill not found: {name}")
