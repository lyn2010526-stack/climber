"""Skill module: chart_generator - Generate charts from data."""
...
class ChartGeneratorInputType(StrEnum):
    TEXT = "text"
    JSON = "json"
    FILE = "file"
    URL = "url"
...
class ChartGeneratorOutputType(StrEnum):
    TEXT = "text"
    JSON = "json"
    HTML = "html"
    MARKDOWN = "markdown"
...
class ChartGeneratorConfig:
    """Configuration for chart_generator skill."""
    enabled: bool = True
...
@dataclass
class ChartGeneratorResult:
    """Result of chart_generator skill execution."""
    success: bool
    data: Any = None
    error: str | None = None
    duration_ms: float = 0
...
@dataclass
class ChartGeneratorMetrics:
    """Metrics tracking for chart_generator skill."""
    total_calls: int = 0
...
class ChartGeneratorSkill:
    ...
    def __init__(self, config: ChartGeneratorConfig | None = None):
        self.config = config or ChartGeneratorConfig()
        self.metrics = ChartGeneratorMetrics()
    ...
    async def execute(self, input_data: Any, input_type: str = "text", **kwargs: Any) -> ChartGeneratorResult:
        """Execute the skill."""
        import time
        start = time.time()
        ...
        try:
            ...
            return ChartGeneratorResult(success=True, data=result_data, duration_ms=duration)
        except Exception as e:
            duration = (time.time() - start) * 1000
            self.metrics.record_call(False, duration, type(e).__name__)
            logger.error("ChartGenerator skill failed", error=str(e))
            return ChartGeneratorResult(success=False, error=str(e), duration_ms=duration)

    async def execute_batch(self, items: list[Any], **kwargs: Any) -> list[ChartGeneratorResult]:
...
class ChartGeneratorSkillRegistry:
    """Registry for managing chart_generator skill instances."""

    def __init__(self):
        self._skills: dict[str, ChartGeneratorSkill] = {}
        self._configs: dict[str, ChartGeneratorConfig] = {}

    def register(self, name: str, skill: ChartGeneratorSkill) -> None:
        """Register a skill instance."""
        self._skills[name] = skill
        logger.info("Registered ChartGenerator skill", name=name)

    def get(self, name: str) -> ChartGeneratorSkill | None:
        """Get skill by name."""
    ...
    async def execute(self, name: str, data: Any, **kwargs: Any) -> ChartGeneratorResult:
        """Execute a named skill."""
        skill = self.get(name)
        if not skill:
            return ChartGeneratorResult(success=False, error=f"Skill not found: {name}")
