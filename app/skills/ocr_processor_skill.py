"""Skill module: ocr_processor - Extract text from images."""
...
class OcrProcessorInputType(StrEnum):
    TEXT = "text"
    JSON = "json"
    FILE = "file"
    URL = "url"
...
class OcrProcessorOutputType(StrEnum):
    TEXT = "text"
    JSON = "json"
    HTML = "html"
    MARKDOWN = "markdown"
...
class OcrProcessorConfig:
    """Configuration for ocr_processor skill."""
    enabled: bool = True
...
class OcrProcessorResult:
    """Result of ocr_processor skill execution."""
    success: bool
    data: Any = None
    error: str | None = None
    duration_ms: float = 0
...
@dataclass
class OcrProcessorMetrics:
    """Metrics tracking for ocr_processor skill."""
    total_calls: int = 0
...
class OcrProcessorSkill:
    ...
    def __init__(self, config: OcrProcessorConfig | None = None):
        self.config = config or OcrProcessorConfig()
        self.metrics = OcrProcessorMetrics()
    ...
    async def execute(self, input_data: Any, input_type: str = "text", **kwargs: Any) -> OcrProcessorResult:
        """Execute the skill."""
        import time
        start = time.time()
        ...
        try:
            ...
            return OcrProcessorResult(success=True, data=result_data, duration_ms=duration)
        except Exception as e:
            duration = (time.time() - start) * 1000
            self.metrics.record_call(False, duration, type(e).__name__)
            logger.error("OcrProcessor skill failed", error=str(e))
            return OcrProcessorResult(success=False, error=str(e), duration_ms=duration)

    async def execute_batch(self, items: list[Any], **kwargs: Any) -> list[OcrProcessorResult]:
...
class OcrProcessorSkillRegistry:
    """Registry for managing ocr_processor skill instances."""

    def __init__(self):
        self._skills: dict[str, OcrProcessorSkill] = {}
        self._configs: dict[str, OcrProcessorConfig] = {}

    def register(self, name: str, skill: OcrProcessorSkill) -> None:
        """Register a skill instance."""
        self._skills[name] = skill
        logger.info("Registered OcrProcessor skill", name=name)

    def get(self, name: str) -> OcrProcessorSkill | None:
        """Get skill by name."""
    ...
    async def execute(self, name: str, data: Any, **kwargs: Any) -> OcrProcessorResult:
        """Execute a named skill."""
        skill = self.get(name)
        if not skill:
            return OcrProcessorResult(success=False, error=f"Skill not found: {name}")
