...
class EmailEnvironment(StrEnum):
    """Environment enum."""
...
class EmailAppConfig:
    ...
    database: EmailDatabaseConfig = field(default_factory=EmailDatabaseConfig)
    cache: EmailCacheConfig = field(default_factory=EmailCacheConfig)
    auth: EmailAuthConfig = field(default_factory=EmailAuthConfig)
    logging: EmailLoggingConfig = field(default_factory=EmailLoggingConfig)
    server: EmailServerConfig = field(default_factory=EmailServerConfig)
    email: EmailEmailConfig = field(default_factory=EmailEmailConfig)
    storage: EmailStorageConfig = field(default_factory=EmailStorageConfig)
    features: EmailFeatureFlags = field(default_factory=EmailFeatureFlags)

    @classmethod
    def from_env(cls) -> EmailAppConfig:
