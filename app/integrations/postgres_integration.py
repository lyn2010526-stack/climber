"""Integration module: postgres - External service connector."""
...
class PostgresAuthType(StrEnum):
...
class PostgresConfig:
    """Configuration for postgres integration."""
    base_url: str = ""
    api_key: str = ""
    api_secret: str = ""
    ...
    retry_policy: PostgresRetryPolicy = field(default_factory=PostgresRetryPolicy)


@dataclass
class PostgresResponse:
    """Standardized response from postgres."""
    status_code: int
...
class PostgresClient:
    """Client for postgres integration."""
    ...
    async def request(
        ...
        if use_cache and method == "GET" and cache_key in self._cache:
            ...
            if time.time() < expires:
                return PostgresResponse(status_code=200, headers={}, body=cached_data, duration_ms=0, cached=True)
        ...
        return PostgresResponse(status_code=0, headers={}, body=str(last_error), duration_ms=0)

    async def get(self, path: str, **kwargs) -> PostgresResponse:
        """Make GET request."""
        return await self.request("GET", path, **kwargs)

    async def post(self, path: str, **kwargs) -> PostgresResponse:
        """Make POST request."""
        return await self.request("POST", path, **kwargs)

    async def put(self, path: str, **kwargs) -> PostgresResponse:
        """Make PUT request."""
        return await self.request("PUT", path, **kwargs)

    async def delete(self, path: str, **kwargs) -> PostgresResponse:
        """Make DELETE request."""
...
class PostgresWebhookHandler:
    """Handle incoming webhooks from postgres."""
