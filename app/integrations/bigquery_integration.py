"""Integration module: bigquery - External service connector."""
...
class BigQueryConfig:
    ...
    retry_policy: BigQueryRetryPolicy = field(default_factory=BigQueryRetryPolicy)


@dataclass
class BigQueryResponse:
    """Standardized response from bigquery."""
    status_code: int
...
class BigQueryClient:
    """Client for bigquery integration."""
    ...
    async def request(
        ...
        if use_cache and method == "GET" and cache_key in self._cache:
            ...
            if time.time() < expires:
                return BigQueryResponse(status_code=200, headers={}, body=cached_data, duration_ms=0, cached=True)
        ...
        return BigQueryResponse(status_code=0, headers={}, body=str(last_error), duration_ms=0)

    async def get(self, path: str, **kwargs) -> BigQueryResponse:
        """Make GET request."""
        return await self.request("GET", path, **kwargs)

    async def post(self, path: str, **kwargs) -> BigQueryResponse:
        """Make POST request."""
        return await self.request("POST", path, **kwargs)

    async def put(self, path: str, **kwargs) -> BigQueryResponse:
        """Make PUT request."""
        return await self.request("PUT", path, **kwargs)

    async def delete(self, path: str, **kwargs) -> BigQueryResponse:
        """Make DELETE request."""
...
class BigQueryWebhookHandler:
    """Handle incoming webhooks from bigquery."""
