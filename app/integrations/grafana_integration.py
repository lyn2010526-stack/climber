"""Integration module: grafana - External service connector."""
...
class GrafanaConfig:
    ...
    retry_policy: GrafanaRetryPolicy = field(default_factory=GrafanaRetryPolicy)


@dataclass
class GrafanaResponse:
    """Standardized response from grafana."""
    status_code: int
...
class GrafanaClient:
    """Client for grafana integration."""
    ...
    async def request(
        ...
        if use_cache and method == "GET" and cache_key in self._cache:
            ...
            if time.time() < expires:
                return GrafanaResponse(status_code=200, headers={}, body=cached_data, duration_ms=0, cached=True)
        ...
        return GrafanaResponse(status_code=0, headers={}, body=str(last_error), duration_ms=0)

    async def get(self, path: str, **kwargs) -> GrafanaResponse:
        """Make GET request."""
        return await self.request("GET", path, **kwargs)

    async def post(self, path: str, **kwargs) -> GrafanaResponse:
        """Make POST request."""
        return await self.request("POST", path, **kwargs)

    async def put(self, path: str, **kwargs) -> GrafanaResponse:
        """Make PUT request."""
        return await self.request("PUT", path, **kwargs)

    async def delete(self, path: str, **kwargs) -> GrafanaResponse:
        """Make DELETE request."""
...
class GrafanaWebhookHandler:
    """Handle incoming webhooks from grafana."""
