"""Integration module: gcp_storage - External service connector."""
...
class GcpStorageAuthType(StrEnum):
...
class GcpStorageConfig:
    """Configuration for gcp_storage integration."""
    base_url: str = ""
    api_key: str = ""
    api_secret: str = ""
    ...
    retry_policy: GcpStorageRetryPolicy = field(default_factory=GcpStorageRetryPolicy)


@dataclass
class GcpStorageResponse:
    """Standardized response from gcp_storage."""
    status_code: int
...
class GcpStorageClient:
    """Client for gcp_storage integration."""
    ...
    async def request(
        ...
        if use_cache and method == "GET" and cache_key in self._cache:
            ...
            if time.time() < expires:
                return GcpStorageResponse(status_code=200, headers={}, body=cached_data, duration_ms=0, cached=True)
        ...
        return GcpStorageResponse(status_code=0, headers={}, body=str(last_error), duration_ms=0)

    async def get(self, path: str, **kwargs) -> GcpStorageResponse:
        """Make GET request."""
        return await self.request("GET", path, **kwargs)

    async def post(self, path: str, **kwargs) -> GcpStorageResponse:
        """Make POST request."""
        return await self.request("POST", path, **kwargs)

    async def put(self, path: str, **kwargs) -> GcpStorageResponse:
        """Make PUT request."""
        return await self.request("PUT", path, **kwargs)

    async def delete(self, path: str, **kwargs) -> GcpStorageResponse:
        """Make DELETE request."""
...
class GcpStorageWebhookHandler:
    """Handle incoming webhooks from gcp_storage."""
