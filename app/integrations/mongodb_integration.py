"""Integration module: mongodb - External service connector."""
...
class MongoDBConfig:
    ...
    retry_policy: MongoDBRetryPolicy = field(default_factory=MongoDBRetryPolicy)


@dataclass
class MongoDBResponse:
    """Standardized response from mongodb."""
    status_code: int
...
class MongoDBClient:
    """Client for mongodb integration."""
    ...
    async def request(
        ...
        if use_cache and method == "GET" and cache_key in self._cache:
            ...
            if time.time() < expires:
                return MongoDBResponse(status_code=200, headers={}, body=cached_data, duration_ms=0, cached=True)
        ...
        return MongoDBResponse(status_code=0, headers={}, body=str(last_error), duration_ms=0)

    async def get(self, path: str, **kwargs) -> MongoDBResponse:
        """Make GET request."""
        return await self.request("GET", path, **kwargs)

    async def post(self, path: str, **kwargs) -> MongoDBResponse:
        """Make POST request."""
        return await self.request("POST", path, **kwargs)

    async def put(self, path: str, **kwargs) -> MongoDBResponse:
        """Make PUT request."""
        return await self.request("PUT", path, **kwargs)

    async def delete(self, path: str, **kwargs) -> MongoDBResponse:
        """Make DELETE request."""
...
class MongoDBWebhookHandler:
    """Handle incoming webhooks from mongodb."""
