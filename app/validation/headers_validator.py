...
class HeadersValidationResult:
    """Validation result."""
    valid: bool = True
    errors: list[HeadersValidationError] = field(default_factory=list)

    def add_error(self, field: str, message: str, code: str = 'invalid') -> None:
        self.valid = False
        self.errors.append(HeadersValidationError(field=field, message=message, code=code))
...
class HeadersValidator:
    ...
    def validate(self, data: dict[str, Any]) -> HeadersValidationResult:
        """Validate data."""
        result = HeadersValidationResult()
