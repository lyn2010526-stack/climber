...
@dataclass
class CookiesValidationResult:
    """Validation result."""
    valid: bool = True
    errors: list[CookiesValidationError] = field(default_factory=list)

    def add_error(self, field: str, message: str, code: str = 'invalid') -> None:
        self.valid = False
        self.errors.append(CookiesValidationError(field=field, message=message, code=code))
...
class CookiesValidator:
    ...
    def validate(self, data: dict[str, Any]) -> CookiesValidationResult:
        """Validate data."""
        result = CookiesValidationResult()
