"""CLI: document - Command line interface."""
...
@document_cli.command(name='list')
@click.option('--page', default=1, help='Page number')
@click.option('--page-size', default=20, help='Items per page')
@click.option('--status', default=None, help='Filter by status')
@click.option('--format', 'output_format', default='table', type=click.Choice(['table', 'json']))
def list_document(page: int, page_size: int, status: str | None, output_format: str):
    """List items."""
...
@document_cli.command(name='create')
...
def create_document(name: str, description: str, status: str):
    """Create item."""
...
@document_cli.command(name='update')
@click.argument('id', type=int)
@click.option('--name', default=None, help='New name')
@click.option('--description', default=None, help='New description')
@click.option('--status', default=None, help='New status')
def update_document(id: int, name: str | None, description: str | None, status: str | None):
    """Update item."""
...
@document_cli.command(name='delete')
@click.argument('id', type=int)
...
@document_cli.command(name='export')
@click.argument('output_file', type=click.Path())
@click.option('--format', 'output_format', default='json', type=click.Choice(['json', 'csv']))
def export_document(output_file: str, output_format: str):
    """Export items."""
    console.print(f'[green]Exported to {output_file}[/green]')


@document_cli.command(name='import')
@click.argument('input_file', type=click.Path(exists=True))
@click.option('--dry-run', is_flag=True, help='Preview changes')
def import_document(input_file: str, dry_run: bool):
    """Import items."""
