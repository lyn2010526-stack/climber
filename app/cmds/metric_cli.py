"""CLI: metric - Command line interface."""
...
@metric_cli.command(name='get')
@click.argument('id', type=int)
def get_metric(id: int):
    """Get item by ID."""
    item = {'id': id, 'name': f'Item {id}', 'status': 'active'}
    console.print_json(data=item)


@metric_cli.command(name='create')
...
def create_metric(name: str, description: str, status: str):
    """Create item."""
...
@metric_cli.command(name='update')
@click.argument('id', type=int)
@click.option('--name', default=None, help='New name')
@click.option('--description', default=None, help='New description')
@click.option('--status', default=None, help='New status')
def update_metric(id: int, name: str | None, description: str | None, status: str | None):
    """Update item."""
...
@metric_cli.command(name='delete')
@click.argument('id', type=int)
...
@metric_cli.command(name='export')
@click.argument('output_file', type=click.Path())
@click.option('--format', 'output_format', default='json', type=click.Choice(['json', 'csv']))
def export_metric(output_file: str, output_format: str):
    """Export items."""
    console.print(f'[green]Exported to {output_file}[/green]')


@metric_cli.command(name='import')
@click.argument('input_file', type=click.Path(exists=True))
@click.option('--dry-run', is_flag=True, help='Preview changes')
def import_metric(input_file: str, dry_run: bool):
    """Import items."""
