"""CLI: blocker - Command line interface."""
...
@blocker_cli.command(name='get')
@click.argument('id', type=int)
def get_blocker(id: int):
    """Get item by ID."""
    item = {'id': id, 'name': f'Item {id}', 'status': 'active'}
    console.print_json(data=item)


@blocker_cli.command(name='create')
...
def create_blocker(name: str, description: str, status: str):
    """Create item."""
...
@blocker_cli.command(name='update')
@click.argument('id', type=int)
@click.option('--name', default=None, help='New name')
@click.option('--description', default=None, help='New description')
@click.option('--status', default=None, help='New status')
def update_blocker(id: int, name: str | None, description: str | None, status: str | None):
    """Update item."""
...
@blocker_cli.command(name='delete')
@click.argument('id', type=int)
@click.confirmation_option(prompt='Are you sure?')
def delete_blocker(id: int):
    """Delete item."""
    console.print(f'[red]Deleted {id}[/red]')


@blocker_cli.command(name='export')
@click.argument('output_file', type=click.Path())
@click.option('--format', 'output_format', default='json', type=click.Choice(['json', 'csv']))
def export_blocker(output_file: str, output_format: str):
    """Export items."""
    console.print(f'[green]Exported to {output_file}[/green]')


@blocker_cli.command(name='import')
@click.argument('input_file', type=click.Path(exists=True))
@click.option('--dry-run', is_flag=True, help='Preview changes')
def import_blocker(input_file: str, dry_run: bool):
    """Import items."""
