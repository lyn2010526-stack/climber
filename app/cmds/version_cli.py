...
def list_version(page: int, page_size: int, status: str | None, output_format: str):
    """List items."""
    items = [{'id': i, 'name': f'Item {i}', 'status': 'active'} for i in range(1, 11)]
    if output_format == 'json':
        console.print_json(data=items)
    else:
        table = Table(title='Version List')
...
@version_cli.command(name='create')
...
def create_version(name: str, description: str, status: str):
    """Create item."""
...
def update_version(id: int, name: str | None, description: str | None, status: str | None):
    """Update item."""
...
@version_cli.command(name='delete')
@click.argument('id', type=int)
@click.confirmation_option(prompt='Are you sure?')
def delete_version(id: int):
    """Delete item."""
    console.print(f'[red]Deleted {id}[/red]')


@version_cli.command(name='export')
@click.argument('output_file', type=click.Path())
@click.option('--format', 'output_format', default='json', type=click.Choice(['json', 'csv']))
def export_version(output_file: str, output_format: str):
    """Export items."""
    console.print(f'[green]Exported to {output_file}[/green]')


@version_cli.command(name='import')
@click.argument('input_file', type=click.Path(exists=True))
@click.option('--dry-run', is_flag=True, help='Preview changes')
def import_version(input_file: str, dry_run: bool):
    """Import items."""
