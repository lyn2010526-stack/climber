"""CLI: decision - Command line interface."""
...
def list_decision(page: int, page_size: int, status: str | None, output_format: str):
    """List items."""
    items = [{'id': i, 'name': f'Item {i}', 'status': 'active'} for i in range(1, 11)]
    if output_format == 'json':
        console.print_json(data=items)
    else:
        table = Table(title='Decision List')
...
@decision_cli.command(name='create')
...
def create_decision(name: str, description: str, status: str):
    """Create item."""
...
@decision_cli.command(name='update')
@click.argument('id', type=int)
@click.option('--name', default=None, help='New name')
@click.option('--description', default=None, help='New description')
@click.option('--status', default=None, help='New status')
def update_decision(id: int, name: str | None, description: str | None, status: str | None):
    """Update item."""
...
@decision_cli.command(name='delete')
@click.argument('id', type=int)
...
@decision_cli.command(name='export')
@click.argument('output_file', type=click.Path())
@click.option('--format', 'output_format', default='json', type=click.Choice(['json', 'csv']))
def export_decision(output_file: str, output_format: str):
    """Export items."""
    console.print(f'[green]Exported to {output_file}[/green]')


@decision_cli.command(name='import')
@click.argument('input_file', type=click.Path(exists=True))
@click.option('--dry-run', is_flag=True, help='Preview changes')
def import_decision(input_file: str, dry_run: bool):
    """Import items."""
